...
function compareUnitsByProps(conn, objUnitProps1, objUnitProps2, handleResult) {
    if (objUnitProps1.unit === objUnitProps2.unit) { return handleResult(0); }
    if (objUnitProps1.level === objUnitProps2.level) { return handleResult(null); }
    if (objUnitProps1.is_free === 1 && objUnitProps2.is_free === 1) // free units
        { return handleResult(null); }

    // genesis
    if (objUnitProps1.latest_included_mc_index === null) { return handleResult(-1); }
    if (objUnitProps2.latest_included_mc_index === null) { return handleResult(+1); }

    if (objUnitProps1.latest_included_mc_index >= objUnitProps2.main_chain_index && objUnitProps2.main_chain_index !== null) { return handleResult(+1); }
    if (objUnitProps2.latest_included_mc_index >= objUnitProps1.main_chain_index && objUnitProps1.main_chain_index !== null) { return handleResult(-1); }
        ...
        conn.query(
            ...
            (rows) => {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    if (objUnitProps.unit === objEarlierUnit.unit) { return handleResult(resultIfFound); }
                    if (objUnitProps.is_on_main_chain === 0 && objUnitProps.level > objEarlierUnit.level) { arrNewStartUnits.push(objUnitProps.unit); }
                }
    ...
    function goDown(arrStartUnits) {
        conn.query(
            ...
            (rows) => {
                ...
                for (let i = 0; i < rows.length; i++) {
                    const objUnitProps = rows[i];
                    if (objUnitProps.unit === objLaterUnit.unit) { return handleResult(resultIfFound); }
                    if (objUnitProps.is_on_main_chain === 0 && objUnitProps.level < objLaterUnit.level) { arrNewStartUnits.push(objUnitProps.unit); }
                }
...
function determineIfIncluded(conn, earlier_unit, arrLaterUnits, handleResult) {
    if (!earlier_unit) { throw Error('no earlier_unit'); }
    ...
    storage.readPropsOfUnits(conn, earlier_unit, arrLaterUnits, (objEarlierUnitProps, arrLaterUnitProps) => {
        ...
        if (objEarlierUnitProps.main_chain_index !== null && max_later_limci >= objEarlierUnitProps.main_chain_index) { return handleResult(true); }

        const max_later_level = Math.max.apply(
            null, arrLaterUnitProps.map(objLaterUnitProps => objLaterUnitProps.level));
        if (max_later_level < objEarlierUnitProps.level) { return handleResult(false); }
        ...
        function goUp(arrStartUnits) {
            ...
            conn.query(
                ...
                (rows) => {
                    ...
                    for (let i = 0; i < rows.length; i++) {
                        const objUnitProps = rows[i];
                        if (objUnitProps.unit === earlier_unit) { return handleResult(true); }
                        if (objUnitProps.is_on_main_chain === 0 && objUnitProps.level > objEarlierUnitProps.level) { arrNewStartUnits.push(objUnitProps.unit); }
                    }
...
function determineIfIncludedOrEqual(conn, earlier_unit, arrLaterUnits, handleResult) {
    if (arrLaterUnits.indexOf(earlier_unit) >= 0) { return handleResult(true); }
        ...
        conn.query(
            ...
            (rows) => {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    if (objUnitProps.author_in_list) { arrUnits.push(objUnitProps.unit); }
                }
...
//        [objEarlierUnitProps.main_chain_index, to_main_chain_index],
        (rows) => {
            arrUnits = rows.map(row => row.unit);
...
function readDescendantUnitsBeforeLandingOnMc(conn, objEarlierUnitProps, arrLaterUnitProps, handleUnits) {
    const max_later_limci = Math.max.apply(null, arrLaterUnitProps.map(objLaterUnitProps => objLaterUnitProps.latest_included_mc_index));
    const max_later_level = Math.max.apply(null, arrLaterUnitProps.map(objLaterUnitProps => objLaterUnitProps.level));
            ...
            (rows) => {
                ...
                for (let i = 0; i < rows.length; i++) {
                    ...
                    if (objUnitProps.main_chain_index !== null && objUnitProps.main_chain_index <= max_later_limci) // exclude free balls!
                        { arrLandedUnits.push(objUnitProps.unit); } else { arrUnlandedUnits.push(objUnitProps.unit); }
                }
...
function readAscendantUnitsAfterTakingOffMc(conn, objEarlierUnitProps, arrLaterUnitProps, handleUnits) {
    const arrLaterUnits = arrLaterUnitProps.map(objLaterUnitProps => objLaterUnitProps.unit);
    const max_later_limci = Math.max.apply(null, arrLaterUnitProps.map(objLaterUnitProps => objLaterUnitProps.latest_included_mc_index));
    ...
    arrLaterUnitProps.forEach((objUnitProps) => {
        if (objUnitProps.latest_included_mc_index >= objEarlierUnitProps.main_chain_index) { arrLandedUnits.push(objUnitProps.unit); } else { arrUnlandedUnits.push(objUnitProps.unit); }
    });
