...
function validateAndSavePrivatePaymentChain(arrPrivateElements, callbacks) {
    if (!ValidationUtils.isNonemptyArray(arrPrivateElements)) { return callbacks.ifError('no priv elements array'); }
    const headElement = arrPrivateElements[0];
    if (!headElement.payload) { return callbacks.ifError('no payload in head element'); }
    const asset = headElement.payload.asset;
    if (!asset) { return callbacks.ifError('no asset in head element'); }
    if (!ValidationUtils.isNonnegativeInteger(headElement.message_index)) { return callbacks.ifError('no message index in head private element'); }

    const validateAndSave = function () {
        storage.readAsset(db, asset, null, (err, objAsset) => {
            if (err) { return callbacks.ifError(err); }
            if (!!objAsset.fixed_denominations !== !!headElement.payload.denomination) { return callbacks.ifError("presence of denomination field doesn't match the asset type"); }
            db.takeConnectionFromPool((conn) => {
                conn.query('BEGIN', () => {
                    ...
                    if (objAsset.fixed_denominations) {
                        if (!ValidationUtils.isNonnegativeInteger(headElement.output_index)) { return transaction_callbacks.ifError('no output index in head private element'); }
                    ...
                    conn.query(
                        ...
                        params,
                        (rows) => {
                            if (rows.length > 1) { throw Error(`more than one output ${sql} ${params.join(', ')}`); }
                            ...
                            const assetModule = objAsset.fixed_denominations ? indivisibleAsset : divisibleAsset;
    ...
    if (conf.bLight) {
 findUnfinishedPastUnitsOfPrivateChains([arrPrivateElements], false, (arrUnfinishedUnits) => {
