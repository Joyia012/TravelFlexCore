...
function saveJoint(objJoint, objValidationState, preCommitCallback, onDone) {
  ...
  db.takeConnectionFromPool((conn) => {
    ...
    for (var i = 0; i < objValidationState.arrAdditionalQueries.length; i++) {
      const objAdditionalQuery = objValidationState.arrAdditionalQueries[i];
    ...
    let values = '?,?,?,?,?,?,?,?,?';
    const params = [objUnit.unit, objUnit.version, objUnit.alt, objUnit.witness_list_unit, objUnit.last_ball_unit,
 ...
 for (var i = 0; i < objJoint.skiplist_units.length; i++) { conn.addQuery(arrQueries, 'INSERT INTO skiplist_units (unit, skiplist_unit) VALUES (?,?)', [objUnit.unit, objJoint.skiplist_units[i]]); }
}
    }

    if (objUnit.parent_units) {
      for (var i = 0; i < objUnit.parent_units.length; i++) { conn.addQuery(arrQueries, 'INSERT INTO parenthoods (child_unit, parent_unit) VALUES(?,?)', [objUnit.unit, objUnit.parent_units[i]]); }
    }
...
conn.addQuery(arrQueries,
      ...
      conn.addQuery(arrQueries, 'UPDATE units SET is_free=0 WHERE unit IN(?)', [objUnit.parent_units], (result) => {
        // in sqlite3, result.affectedRows actually returns the number of _matched_ rows
        const count_consumed_free_units = result.affectedRows;
        console.log(`${count_consumed_free_units} free units consumed`);
        objUnit.parent_units.forEach((parent_unit) => {
          if (storage.assocUnstableUnits[parent_unit]) { storage.assocUnstableUnits[parent_unit].is_free = 0; }
        });
      });
    }
    ...
    const arrAuthorAddresses = [];
    for (var i = 0; i < objUnit.authors.length; i++) {
      const author = objUnit.authors[i];
      ...
      if (definition) {
        ...
        if (definition_chash === author.address) { conn.addQuery(arrQueries, `INSERT ${conn.getIgnore()} INTO addresses (address) VALUES(?)`, [author.address]); }
      } else if (objUnit.content_hash) { conn.addQuery(arrQueries, `INSERT ${conn.getIgnore()} INTO addresses (address) VALUES(?)`, [author.address]); }
 ...
 conn.addQuery(arrQueries, 'INSERT INTO authentifiers (unit, address, path, authentifier) VALUES(?,?,?,?)',
    ...
    if (!objUnit.content_hash) {
      for (var i = 0; i < objUnit.messages.length; i++) {
        ...
        if (message.app === 'text') { text_payload = message.payload; } else if (message.app === 'data' || message.app === 'profile' || message.app === 'attestation' || message.app === 'definition_template') { text_payload = JSON.stringify(message.payload); }
 ...
 conn.addQuery(arrQueries,
        ...
        if ('spend_proofs' in message) {
          for (var j = 0; j < message.spend_proofs.length; j++) {
            const objSpendProof = message.spend_proofs[j];
            conn.addQuery(arrQueries,
    ...
    function determineInputAddressFromSrcOutput(asset, denomination, input, handleAddress) {
      conn.query(
        ...
        (rows) => {
          if (rows.length > 1) { throw Error('multiple src outputs found'); }
          if (rows.length === 0) {
          ...
          const row = rows[0];
          if (!(!asset && !row.asset || asset === row.asset)) { throw Error("asset doesn't match"); }
          if (denomination !== row.denomination) { throw Error("denomination doesn't match"); }
          const address = row.address;
          if (arrAuthorAddresses.indexOf(address) === -1) { throw Error('src output address not among authors'); }
    ...
    function addInlinePaymentQueries(cb) {
      async.forEachOfSeries(
        objUnit.messages,
        (message, i, cb2) => {
          if (message.payload_location !== 'inline') { return cb2(); }
          const payload = message.payload;
          if (message.app !== 'payment') { return cb2(); }
          ...
          async.forEachOfSeries(
            ...
            (input, j, cb3) => {
              ...
              const src_unit = (type === 'transfer') ? input.unit : null;
              const src_message_index = (type === 'transfer') ? input.message_index : null;
              const src_output_index = (type === 'transfer') ? input.output_index : null;
              const from_main_chain_index = (type === 'witnessing' || type === 'headers_commission') ? input.from_main_chain_index : null;
              const to_main_chain_index = (type === 'witnessing' || type === 'headers_commission') ? input.to_main_chain_index : null;

              const determineInputAddress = function (handleAddress) {
                if (type === 'headers_commission' || type === 'witnessing' || type === 'issue') { return handleAddress((arrAuthorAddresses.length === 1) ? arrAuthorAddresses[0] : input.address); }
                // hereafter, transfer
                if (arrAuthorAddresses.length === 1) { return handleAddress(arrAuthorAddresses[0]); }
                determineInputAddressFromSrcOutput(payload.asset, denomination, input, handleAddress);
              };

              determineInputAddress((address) => {
                const is_unique =
                  objValidationState.arrDoubleSpendInputs.some(ds => (ds.message_index === i && ds.input_index === j))
                    ? null : 1;
    ...
    function updateBestParent(cb) {
      ...
      conn.query(
        ...
        (rows) => {
          if (rows.length !== 1) { throw Error('zero or more than one best parent unit?'); }
          my_best_parent_unit = rows[0].unit;
          if (my_best_parent_unit !== objValidationState.best_parent_unit) { throwError(`different best parents, validation: ${objValidationState.best_parent_unit}, writer: ${my_best_parent_unit}`); }
          conn.query('UPDATE units SET best_parent_unit=? WHERE unit=?', [my_best_parent_unit, objUnit.unit], () => {
            cb();
          });
        }
      );
    ...
    function determineMaxLevel(handleMaxLevel) {
      ...
      async.each(
        ...
        (parent_unit, cb) => {
          storage.readStaticUnitProps(conn, parent_unit, (props) => {
            if (props.level > max_level) { max_level = props.level; }
            cb();
          });
        },
        () => {
    ...
    function updateLevel(cb) {
      conn.query('SELECT MAX(level) AS max_level FROM units WHERE unit IN(?)', [objUnit.parent_units], (rows) => {
        if (rows.length !== 1) { throw Error('not a single max level?'); }
        determineMaxLevel((max_level) => {
          if (max_level !== rows[0].max_level) { throwError(`different max level, sql: ${rows[0].max_level}, props: ${max_level}`); }
          objNewUnitProps.level = max_level + 1;
          conn.query('UPDATE units SET level=? WHERE unit=?', [rows[0].max_level + 1, objUnit.unit], () => {
            cb();
          });
        });
      });
    ...
    function updateWitnessedLevel(cb) {
      if (objUnit.witnesses) { updateWitnessedLevelByWitnesslist(objUnit.witnesses, cb); } else {
 storage.readWitnessList(conn, objUnit.witness_list_unit, (arrWitnesses) => {
    ...
    function updateWitnessedLevelByWitnesslist(arrWitnesses, cb) {
      ...
      function setWitnessedLevel(witnessed_level) {
        profiler.start();
        if (witnessed_level !== objValidationState.witnessed_level) { throwError(`different witnessed levels, validation: ${objValidationState.witnessed_level}, writer: ${witnessed_level}`); }
        objNewUnitProps.witnessed_level = witnessed_level;
        conn.query('UPDATE units SET witnessed_level=? WHERE unit=?', [witnessed_level, objUnit.unit], () => {
      ...
      function addWitnessesAndGoUp(start_unit) {
        profiler.start();
        storage.readStaticUnitProps(conn, start_unit, (props) => {
          ...
          profiler.start();
          storage.readUnitAuthors(conn, start_unit, (arrAuthors) => {
...
arrOps.push((cb) => {
          ...
          async.series(arrOps, (err) => {
            ...
            conn.query(err ? 'ROLLBACK' : 'COMMIT', () => {
              ...
              profiler.increment();
              if (err) { storage.resetUnstableUnits(unlock); } else { unlock(); }
              if (!err) { eventBus.emit(`saved_unit-${objUnit.unit}`, objJoint); }
              if (onDone) { onDone(err); }
              count_writes++;
              if (conf.storage === 'sqlite') { updateSqliteStats(); }
            });
          });
        });
      });
...
function readCountOfAnalyzedUnits(handleCount) {
  if (count_units_in_prev_analyze) { return handleCount(count_units_in_prev_analyze); }
  db.query("SELECT * FROM sqlite_master WHERE type='table' AND name='sqlite_stat1'", (rows) => {
    if (rows.length === 0) { return handleCount(0); }
    db.query("SELECT stat FROM sqlite_stat1 WHERE tbl='units' AND idx='sqlite_autoindex_units_1'", (rows) => {
      if (rows.length !== 1) {
...
function updateSqliteStats() {
  ...
  if (count_writes % 100 !== 0) { return; }
  if (count_writes % 1000 === 0) {
    const total_time = (Date.now() - start_time) / 1000;
    const recent_time = (Date.now() - prev_time) / 1000;
    const recent_tps = 1000 / recent_time;
    const avg_tps = count_writes / total_time;
    prev_time = Date.now();
    //	console.error(count_writes+" units done in "+total_time+" s, recent "+recent_tps+" tps, avg "+avg_tps+" tps");
  }
  db.query('SELECT MAX(rowid) AS count_units FROM units', (rows) => {
    const count_units = rows[0].count_units;
    if (count_units > 500000) // the db is too big
      { return; }
    readCountOfAnalyzedUnits((count_analyzed_units) => {
      console.log(`count analyzed units: ${count_analyzed_units}`);
      if (count_units < 2 * count_analyzed_units) { return; }
      count_units_in_prev_analyze = count_units;
      console.log('will update sqlite stats');
      db.query('ANALYZE', () => {
        db.query('ANALYZE sqlite_master', () => {
