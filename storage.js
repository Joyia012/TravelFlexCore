...
function readJoint(conn, unit, callbacks) {
  if (!conf.bSaveJointJson) { return readJointDirectly(conn, unit, callbacks); }
  conn.query('SELECT json FROM joints WHERE unit=?', [unit], (rows) => {
    if (rows.length === 0) { return readJointDirectly(conn, unit, callbacks); }
...
function readJointDirectly(conn, unit, callbacks, bRetrying) {
  ...
  conn.query(
    ...
    (unit_rows) => {
      ...
      if (!conf.bLight && !objUnit.last_ball) { throw Error(`no last ball in unit ${JSON.stringify(objUnit)}`); }
      ...
      } else { delete objUnit.content_hash; }
      ...
      async.series([
        function (callback) { // parents
          conn.query(
            ...
            (rows) => {
              ...
              objUnit.parent_units = rows.map(row => row.parent_unit);
              callback();
            }
          );
        },
        function (callback) { // ball
          if (bRetrievable && !isGenesisUnit(unit)) { return callback(); }
        ...
        function (callback) { // earned_headers_commission_recipients
          ...
          conn.query('SELECT address, earned_headers_commission_share FROM earned_headers_commission_recipients \
            ...
            [unit],
            (rows) => {
              if (rows.length > 0) { objUnit.earned_headers_commission_recipients = rows; }
              callback();
            }
          );
        },
        function (callback) { // authors
          conn.query('SELECT address, definition_chash FROM unit_authors WHERE unit=? ORDER BY address', [unit], (rows) => {
            ...
            async.eachSeries(
              rows,
              (row, cb) => {
                const author = { address: row.address };
                ...
                if (bVoided) { return onAuthorDone(); }
                author.authentifiers = {};
                conn.query(
                  'SELECT path, authentifier FROM authentifiers WHERE unit=? AND address=?',
                  [unit, author.address],
                  (sig_rows) => {
                    for (let i = 0; i < sig_rows.length; i++) { author.authentifiers[sig_rows[i].path] = sig_rows[i].authentifier; }
        ...
        function (callback) { // messages
          ...
          conn.query(
            ...
            (rows) => {
              if (rows.length === 0) {
                if (conf.bLight) { throw new Error(`no messages in unit ${unit}`); }
                ...
                (row, cb) => {
                  ...
                  function addSpendProofs() {
                    conn.query(
                      ...
                      [unit, message_index],
                      (proof_rows) => {
                        if (proof_rows.length === 0) { return cb(); }
                  ...
                  if (objMessage.payload_location !== 'inline') { return addSpendProofs(); }
                  switch (objMessage.app) {
                    case 'address_definition_change':
                      conn.query(
                        ...
                        [unit, message_index],
                        (dch_rows) => {
                          if (dch_rows.length === 0) { throw Error('no definition change?'); }
                    ...
                    case 'poll':
                      conn.query(
                        ...
                        (poll_rows) => {
                          if (poll_rows.length !== 1) { throw Error('no poll question or too many?'); }
                          ...
                          conn.query('SELECT choice FROM poll_choices WHERE unit=? ORDER BY choice_index', [unit], (ch_rows) => {
                            if (ch_rows.length === 0) { throw Error('no choices?'); }
                            objMessage.payload.choices = ch_rows.map(choice_row => choice_row.choice);
                            addSpendProofs();
                          });
                        }
                      );
                    ...
                    case 'vote':
                      conn.query(
                        ...
                        (vote_rows) => {
                          if (vote_rows.length !== 1) { throw Error('no vote choice or too many?'); }
                    ...
                    case 'asset':
                      conn.query(
                        ...
                        [unit, message_index],
                        (asset_rows) => {
                          if (asset_rows.length !== 1) { throw Error('no asset or too many?'); }
                          ...
                          if (objMessage.payload.issue_condition) { objMessage.payload.issue_condition = JSON.parse(objMessage.payload.issue_condition); }
                          if (objMessage.payload.transfer_condition) { objMessage.payload.transfer_condition = JSON.parse(objMessage.payload.transfer_condition); }

                          const addAttestors = function (next) {
                            if (!objMessage.payload.spender_attested) { return next(); }
                            conn.query(
                              'SELECT attestor_address FROM asset_attestors \n\
                              WHERE unit=? AND message_index=? ORDER BY attestor_address',
                              [unit, message_index],
                              (att_rows) => {
                                if (att_rows.length === 0) { throw Error('no attestors?'); }
                                objMessage.payload.attestors = att_rows.map(att_row => att_row.attestor_address);
                                next();
                              }
                            );
                          };

                          const addDenominations = function (next) {
                            if (!objMessage.payload.fixed_denominations) { return next(); }
                            conn.query(
                              'SELECT denomination, count_coins FROM asset_denominations \n\
                              WHERE asset=? ORDER BY denomination',
                              [unit],
                              (denom_rows) => {
                                if (denom_rows.length === 0) { throw Error('no denominations?'); }
                                objMessage.payload.denominations = denom_rows.map((denom_row) => {
                                  const denom = { denomination: denom_row.denomination };
                                  if (denom_row.count_coins) { denom.count_coins = denom_row.count_coins; }
                                  return denom;
                                });
                                next();
                              }
                      ...
                      conn.query(
                        ...
                        (att_rows) => {
                          ...
                          if (att_rows.length > 1
                            && att_rows.some(att_row => (att_row.asset !== objMessage.payload.asset))) { throw Error('different assets in attestor list'); }
                          objMessage.payload.attestors = att_rows.map(att_row => att_row.attestor_address);
                          addSpendProofs();
                        }
                      );
                    ...
                    case 'data_feed':
                      conn.query(
                        ...
                        (df_rows) => {
                          if (df_rows.length === 0) { throw Error('no data feed?'); }
                          objMessage.payload = {};
                          df_rows.forEach((df_row) => {
                        ...
                        conn.query(
                          ...
                          (input_rows) => {
                            ...
                            for (let i = 0; i < input_rows.length; i++) {
                              ...
                              if (i > 0) {
                                if (asset !== prev_asset) { throw Error('different assets in inputs?'); }
                                if (denomination !== prev_denomination) { throw Error('different denominations in inputs?'); }
                              }
                              if (i === 0 && asset) {
                                objMessage.payload.asset = asset;
                                if (input.fixed_denominations) { objMessage.payload.denomination = denomination; }
                              }
                              delete input.asset;
                              delete input.denomination;
                              delete input.fixed_denominations;
                              objectHash.cleanNulls(input);
                              if (input.type === 'transfer' || objUnit.authors.length === 1) { delete input.address; }
                              if (input.type === 'transfer') { delete input.type; }
                        ...
                        conn.query( // we don't select blinding because it's absent on public payments
                          ...
                          (output_rows) => {
                            for (let i = 0; i < output_rows.length; i++) {
                              ...
                              if (output.asset !== prev_asset) { throw Error('different assets in outputs?'); }
                              if (output.denomination !== prev_denomination) { throw Error('different denominations in outputs?'); }
                              delete output.asset;
        ...
        if (!conf.bLight && !isCorrectHash(objUnit, unit)) {
          if (bRetrying) { throw Error(`unit hash verification failed, unit: ${unit}, objUnit: ${JSON.stringify(objUnit)}`); }
          console.log('unit hash verification failed, will retry');
          return setTimeout(() => {
            readJointDirectly(conn, unit, callbacks, true);
          }, 60 * 1000);
        }
        if (!conf.bSaveJointJson || !bStable || (bFinalBad && bRetrievable) || bRetrievable) { return callbacks.ifFound(objJoint); }
...
function readJointWithBall(conn, unit, handleJoint) {
  readJoint(conn, unit, {
    ...
    ifFound(objJoint) {
      if (objJoint.ball) { return handleJoint(objJoint); }
      conn.query('SELECT ball FROM balls WHERE unit=?', [unit], (rows) => {
        if (rows.length === 1) { objJoint.ball = rows[0].ball; }
...
function readWitnessList(conn, unit, handleWitnessList, bAllowEmptyList) {
  ...
  if (arrWitnesses) { return handleWitnessList(arrWitnesses); }
  conn.query('SELECT address FROM unit_witnesses WHERE unit=? ORDER BY address', [unit], (rows) => {
    if (!bAllowEmptyList && rows.length === 0) { throw Error(`witness list of unit ${unit} not found`); }
    if (rows.length > 0 && rows.length !== constants.COUNT_WITNESSES) { throw Error(`wrong number of witnesses in unit ${unit}`); }
    arrWitnesses = rows.map(row => row.address);
    if (rows.length > 0) { assocCachedUnitWitnesses[unit] = arrWitnesses; }
...
function readWitnesses(conn, unit, handleWitnessList) {
  ...
  conn.query('SELECT witness_list_unit FROM units WHERE unit=?', [unit], (rows) => {
    if (rows.length === 0) { throw Error(`unit ${unit} not found`); }
    const witness_list_unit = rows[0].witness_list_unit;
    readWitnessList(conn, witness_list_unit || unit, (arrWitnesses) => {
...
function determineWitnessedLevelAndBestParent(conn, arrParentUnits, arrWitnesses, handleWitnessedLevelAndBestParent) {
  ...
  function addWitnessesAndGoUp(start_unit) {
    readStaticUnitProps(conn, start_unit, (props) => {
      ...
      if (level === null) { throw Error('null level in updateWitnessedLevel'); }
      if (level === 0) // genesis
      ...
      readUnitAuthors(conn, start_unit, (arrAuthors) => {
        for (let i = 0; i < arrAuthors.length; i++) {
          ...
          if (arrWitnesses.indexOf(address) !== -1 && arrCollectedWitnesses.indexOf(address) === -1) { arrCollectedWitnesses.push(address); }
        }
  ...
  determineBestParent(conn, { parent_units: arrParentUnits, witness_list_unit: 'none' }, arrWitnesses, (best_parent_unit) => {
    if (!best_parent_unit) { throw Error(`no best parent of ${arrParentUnits.join(', ')}, witnesses ${arrWitnesses.join(', ')}`); }
...
function readDefinitionByAddress(conn, address, max_mci, callbacks) {
  if (max_mci === null) { max_mci = MAX_INT32; }
...
function readDefinitionAtMci(conn, definition_chash, max_mci, callbacks) {
  ...
  conn.query(sql, params, (rows) => {
    if (rows.length === 0) { return callbacks.ifDefinitionNotFound(definition_chash); }
...
function readUnitProps(conn, unit, handleProps) {
  if (assocStableUnits[unit]) { return handleProps(assocStableUnits[unit]); }
  conn.query(
    ...
    [unit],
    (rows) => {
      if (rows.length !== 1) { throw Error('not 1 row'); }
      const props = rows[0];
      if (props.is_stable) { assocStableUnits[unit] = props; } else {
        const props2 = _.cloneDeep(assocUnstableUnits[unit]);
        if (!props2) { throw Error(`no unstable props of ${unit}`); }
        delete props2.parent_units;
        if (!_.isEqual(props, props2)) { throw Error(`different props of ${unit}, mem: ${JSON.stringify(props2)}, db: ${JSON.stringify(props)}`); }
      }
      handleProps(props);
    }
  );
...
function readPropsOfUnits(conn, earlier_unit, arrLaterUnits, handleProps) {
  ...
  conn.query(
    ...
    (rows) => {
      if (rows.length !== arrLaterUnits.length + (bEarlierInLaterUnits ? 0 : 1)) { throw Error(`wrong number of rows for earlier ${earlier_unit}, later ${arrLaterUnits}`); }
      let objEarlierUnitProps,
arrLaterUnitProps = [];
      for (let i = 0; i < rows.length; i++) {
        if (rows[i].unit === earlier_unit) { objEarlierUnitProps = rows[i]; } else { arrLaterUnitProps.push(rows[i]); }
      }
      if (bEarlierInLaterUnits) { arrLaterUnitProps.push(objEarlierUnitProps); }
...
function readLastStableMcUnitProps(conn, handleLastStableMcUnitProps) {
  conn.query(
    ...
    (rows) => {
      if (rows.length === 0) { return handleLastStableMcUnitProps(null); } // empty database
      // throw "readLastStableMcUnitProps: no units on stable MC?";
      if (!rows[0].ball) { throw Error(`no ball for last stable unit ${rows[0].unit}`); }
...
function findLastBallMciOfMci(conn, mci, handleLastBallMci) {
  if (mci === 0) { throw Error('findLastBallMciOfMci called with mci=0'); }
  conn.query(
    ...
    [mci],
    (rows) => {
      if (rows.length !== 1) { throw Error(`last ball's mci count ${rows.length} !== 1, mci = ${mci}`); }
      if (rows[0].is_on_main_chain !== 1) { throw Error('lb is not on mc?'); }
      ...
      (unit_rows) => {
        ...
        async.eachSeries(
          ...
          (unit_row, cb) => {
            ...
            if (!unit_row.content_hash) { throw Error(`no content hash in bad unit ${unit}`); }
            readJoint(conn, unit, {
              ifNotFound() {
          ...
          () => {
            if (arrQueries.length === 0) { return handleMinRetrievableMci(min_retrievable_mci); }
            async.series(arrQueries, () => {
              unit_rows.forEach((unit_row) => {
...
function initializeMinRetrievableMci() {
  db.query(
    ...
    (rows) => {
      if (rows.length !== 1) { throw Error('MAX() no rows?'); }
      min_retrievable_mci = rows[0].min_retrievable_mci;
      if (min_retrievable_mci === null) { min_retrievable_mci = 0; }
    }
  );
}
...
function archiveJointAndDescendantsIfExists(from_unit) {
  ...
  db.query('SELECT 1 FROM units WHERE unit=?', [from_unit], (rows) => {
    if (rows.length > 0) { archiveJointAndDescendants(from_unit); }
  });
}
  ...
  db.executeInTransaction((conn, cb) => {
      function addChildren(arrParentUnits) {
        conn.query('SELECT DISTINCT child_unit FROM parenthoods WHERE parent_unit IN(?)', [arrParentUnits], (rows) => {
          ...
          const arrChildUnits = rows.map(row => row.child_unit);
...
function readAssetInfo(conn, asset, handleAssetInfo) {
  ...
  if (objAsset) { return handleAssetInfo(objAsset); }
  conn.query(
    ...
    [asset],
    (rows) => {
      if (rows.length > 1) { throw Error('more than one asset?'); }
      if (rows.length === 0) { return handleAssetInfo(null); }
      const objAsset = rows[0];
      if (objAsset.issue_condition) { objAsset.issue_condition = JSON.parse(objAsset.issue_condition); }
      if (objAsset.transfer_condition) { objAsset.transfer_condition = JSON.parse(objAsset.transfer_condition); }
...
function readAsset(conn, asset, last_ball_mci, handleAsset) {
  if (last_ball_mci === null) {
    if (conf.bLight) { last_ball_mci = MAX_INT32; } else {
return readLastStableMcIndex(conn, (last_stable_mci) => {
  ...
  readAssetInfo(conn, asset, (objAsset) => {
    if (!objAsset) { return handleAsset(`asset ${asset} not found`); }
    if (objAsset.main_chain_index > last_ball_mci) { return handleAsset('asset definition must be before last ball'); }
    if (objAsset.sequence !== 'good') { return handleAsset('asset definition is not serial'); }
    if (!objAsset.spender_attested) { return handleAsset(null, objAsset); }
    ...
    conn.query(
      ...
      (latest_rows) => {
        ...
        if (!max_level) { throw Error('no max level of asset attestors'); }
        ...
        conn.query(
          ...
          (att_rows) => {
            if (att_rows.length === 0) { throw Error('no attestors?'); }
            objAsset.arrAttestorAddresses = att_rows.map(att_row => att_row.attestor_address);
...
function filterAttestedAddresses(conn, objAsset, last_ball_mci, arrAddresses, handleAttestedAddresses) {
  conn.query(
    ...
    (addr_rows) => {
      const arrAttestedAddresses = addr_rows.map(addr_row => addr_row.address);
...
function filterNewOrUnstableUnits(arrUnits, handleFilteredUnits) {
  sliceAndExecuteQuery('SELECT unit FROM units WHERE unit IN(?) AND is_stable=1', [arrUnits], arrUnits, (rows) => {
    const arrKnownStableUnits = rows.map(row => row.unit);
...
function determineBestParent(conn, objUnit, arrWitnesses, handleBestParent) {
  ...
  conn.query(
    ...
    (rows) => {
      if (rows.length !== 1) { return handleBestParent(null); }
...
function determineIfHasWitnessListMutationsAlongMc(conn, objUnit, last_ball_unit, arrWitnesses, handleResult) {
  ...
  buildListOfMcUnitsWithPotentiallyDifferentWitnesslists(conn, objUnit, last_ball_unit, arrWitnesses, (bHasBestParent, arrMcUnits) => {
    if (!bHasBestParent) { return handleResult('no compatible best parent'); }
    console.log('###### MC units ', arrMcUnits);
    if (arrMcUnits.length === 0) { return handleResult(); }
    conn.query(
      ...
      (rows) => {
        console.log(rows);
        if (rows.length > 0) { return handleResult(`too many (${constants.COUNT_WITNESSES - rows[0].count_matching_witnesses}) witness list mutations relative to MC unit ${rows[0].unit}`); }
        handleResult();
      }
    );
  });
...
function buildListOfMcUnitsWithPotentiallyDifferentWitnesslists(conn, objUnit, last_ball_unit, arrWitnesses, handleList) {
  function addAndGoUp(unit) {
    readStaticUnitProps(conn, unit, (props) => {
      // the parent has the same witness list and the parent has already passed the MC compatibility test
      if (objUnit.witness_list_unit && objUnit.witness_list_unit === props.witness_list_unit) { return handleList(true, arrMcUnits); }
      arrMcUnits.push(unit);
      if (unit === last_ball_unit) { return handleList(true, arrMcUnits); }
      if (!props.best_parent_unit) { throw Error(`no best parent of unit ${unit}?`); }
  ...
  determineBestParent(conn, objUnit, arrWitnesses, (best_parent_unit) => {
    if (!best_parent_unit) { return handleList(false); }
...
function readUnitAuthors(conn, unit, handleAuthors) {
  ...
  if (arrAuthors) { return handleAuthors(arrAuthors); }
  conn.query('SELECT address FROM unit_authors WHERE unit=?', [unit], (rows) => {
    if (rows.length === 0) { throw Error('no authors'); }
    const arrAuthors2 = rows.map(row => row.address).sort();
...
function isKnownUnit(unit) {
  return !!((assocCachedUnits[unit] || assocKnownUnits[unit]));
}
...
function shrinkCache() {
  if (Object.keys(assocCachedAssetInfos).length > MAX_ITEMS_IN_CACHE) { assocCachedAssetInfos = {}; }
  ...
  if (arrPropsUnits.length < MAX_ITEMS_IN_CACHE && arrAuthorsUnits.length < MAX_ITEMS_IN_CACHE && arrWitnessesUnits.length < MAX_ITEMS_IN_CACHE && arrKnownUnits.length < MAX_ITEMS_IN_CACHE && arrStableUnits.length < MAX_ITEMS_IN_CACHE) { return console.log('cache is small, will not shrink'); }
...
function initUnstableUnits(onDone) {
  db.query(
    ...
    (rows) => {
      ...
      if (Object.keys(assocUnstableUnits).length === 0) { return onDone ? onDone() : null; }
      db.query(
...
if (!conf.bLight) { archiveJointAndDescendantsIfExists('N6QadI9yg3zLxPMphfNGJcPfddW4yHPkoGMbbGZsWa0='); }
