                ...
                async.eachSeries(
                    ...
                    () => {
                        ...
                        arrMemberDeviceAddresses.forEach((device_address) => {
                            if (device_address !== device.getMyDeviceAddress()) { sendOfferToCreateNewSharedAddress(device_address, arrAddressDefinitionTemplate); }
                        });
                    }
                );
            }
            ...
            db.query(
                ...
                (rows) => {
                    ...
                    db.query(
                        ...
                        [address_definition_template_chash],
                        (templ_rows) => {
                            if (templ_rows.length !== 1) { throw Error('template not found'); }
                            ...
                            db.query(
                                ...
                                () => {
                                    ...
                                    async.series(arrQueries, () => {
                                        ...
                                        rows.forEach((row) => {
                                            if (row.device_address !== device.getMyDeviceAddress()) { sendNewSharedAddress(row.device_address, shared_address, arrDefinition, assocSignersByPath); }
                                        });
                                        forwardNewSharedAddressToCosignersOfMyMemberAddresses(shared_address, arrDefinition, assocSignersByPath);
                                        if (conf.bLight) { network.addLightWatchedAddress(shared_address); }
                                    });
                                }
                            );
                        }
    ...
    db.query(
        ...
        () => {
            ...
            async.series(arrQueries, () => {
                ...
                if (conf.bLight) { network.addLightWatchedAddress(address); }
                if (!bForwarded) { forwardNewSharedAddressToCosignersOfMyMemberAddresses(address, arrDefinition, assocSignersByPath); }
                if (onDone) { onDone(); }
            });
        }
    );
...
function determineIfIncludesMeAndRewriteDeviceAddress(assocSignersByPath, handleResult) {
    ...
    db.query(
        ...
        //	handleResult(rows.length === arrMyMemberAddresses.length ? null : "Some of my member addresses not found");
            if (rows.length === 0) { return handleResult('I am not a member of this shared address'); }
...
function handleNewSharedAddress(body, callbacks) {
    ...
    determineIfIncludesMeAndRewriteDeviceAddress(body.signers, (err) => {
        if (err) { return callbacks.ifError(err); }
        validateAddressDefinition(body.definition, (err) => {
            if (err) { return callbacks.ifError(err); }
    ...
    function evaluate(arr, path) {
        ...
        switch (op) {
            ...
            case 'address':
                var address = args;
                var prefix = '$address@';
                if (!ValidationUtils.isNonemptyString(address) || address.substr(0, prefix.length) !== prefix) { return; }
...
function validateAddressDefinitionTemplate(arrDefinitionTemplate, from_address, handleResult) {
    ...
    if (arrDeviceAddresses.length < 2) { return handleResult('less than 2 member devices'); }
...
function readAllControlAddresses(conn, arrAddresses, handleLists) {
    ...
    conn.query(
        ...
        (rows) => {
            ...
            const arrControlAddresses = rows.map(row => row.address);
            const arrControlDeviceAddresses = rows.filter(row => row.have_correspondent).map(row => row.device_address);
...
function readSharedAddressDefinition(shared_address, handleDefinition) {
    db.query(
        ...
        [shared_address],
        (rows) => {
            if (rows.length !== 1) { throw Error(`shared definition not found ${shared_address}`); }
...
function readSharedAddressCosigners(shared_address, handleCosigners) {
    db.query(
        ...
        (rows) => {
            if (rows.length === 0) { throw Error(`no cosigners found for shared address ${shared_address}`); }
...
function readSharedAddressPeerAddresses(shared_address, handlePeerAddresses) {
    db.query(
        ...
        //		throw Error("no peers found for shared address "+shared_address);
            const arrPeerAddresses = rows.map(row => row.address);
