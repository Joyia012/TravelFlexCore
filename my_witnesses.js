...
function readMyWitnesses(handleWitnesses, actionIfEmpty) {
    db.query('SELECT address FROM my_witnesses ORDER BY address', (rows) => {
        let arrWitnesses = rows.map(row => row.address);
        ...
        if (arrWitnesses.length === 0) {
            if (actionIfEmpty === 'ignore') { return handleWitnesses([]); }
        ...
        if (arrWitnesses.length !== constants.COUNT_WITNESSES) { throw Error(`wrong number of my witnesses: ${arrWitnesses.length}`); }
...
function replaceWitness(old_witness, new_witness, handleResult) {
    if (!ValidationUtils.isValidAddress(new_witness)) { return handleResult('new witness address is invalid'); }
    readMyWitnesses((arrWitnesses) => {
        if (arrWitnesses.indexOf(old_witness) === -1) { return handleResult('old witness not known'); }
        if (arrWitnesses.indexOf(new_witness) >= 0) { return handleResult('new witness already present'); }
        ...
        db.query(
            ...
            [new_witness],
            (rows) => {
                if (rows.length === 0) { return handleResult('no stable messages from the new witness yet'); }
                storage.determineIfWitnessAddressDefinitionsHaveReferences(db, [new_witness], (bHasReferences) => {
                    if (bHasReferences) { return handleResult('address definition of the new witness has or had references'); }
                    doReplace();
                });
            }
        );
...
function insertWitnesses(arrWitnesses, onDone) {
    if (arrWitnesses.length !== constants.COUNT_WITNESSES) { throw Error(`attempting to insert wrong number of witnesses: ${arrWitnesses.length}`); }
