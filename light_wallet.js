...
function reconnectToLightVendor() {
    network.findOutboundPeerOrConnect(network.light_vendor_url, (err, ws) => {
        if (err) { return console.log(`reconnectToLightVendor: ${err}`); }
        if (ws.bLightVendor) { return console.log('already connected to light vendor'); }
        if (ws.bRefreshingHistory) { return console.log('already refreshing history'); }
        refreshLightClientHistory();
    });
}
...
function prepareRequestForHistory(handleResult) {
    myWitnesses.readMyWitnesses((arrWitnesses) => {
        ...
        walletGeneral.readMyAddresses((arrAddresses) => {
            if (arrAddresses.length > 0) { objHistoryRequest.addresses = arrAddresses; }
            readListOfUnstableUnits((arrUnits) => {
                if (arrUnits.length > 0) { objHistoryRequest.requested_joints = arrUnits; }
                if (!objHistoryRequest.addresses && !objHistoryRequest.requested_joints) { return handleResult(null); }
                if (!objHistoryRequest.addresses) { return handleResult(objHistoryRequest); }
...
function refreshLightClientHistory() {
    ...
    if (!network.light_vendor_url) { return console.log('refreshLightClientHistory called too early: light_vendor_url not set yet'); }
            ...
            network.sendRequest(ws, 'light/get_history', objRequest, false, (ws, request, response) => {
                ...
                light.processHistory(response, {
                    ...
                    ifOk(bRefreshUI) {
                        clearInterval(interval);
                        finish();
                        if (bRefreshUI) { eventBus.emit('maybe_new_transactions'); }
                    }
                });
            });
        });
...
function archiveDoublespendUnits() {
    db.query(`SELECT unit FROM units WHERE is_stable=0 AND is_free=1 AND creation_date<${db.addTime('-1 DAY')}`,(rows) => {
        ...
        arrUnits.forEach((unit) => {
            network.requestFromLightVendor('get_joint', unit, (ws, request, response) => {
                if (response.error) { return breadcrumbs.add(`get_joint ${unit}: ${response.error}`); }
