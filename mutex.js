...
function isAnyOfKeysLocked(arrKeys) {
    for (let i = 0; i < arrLockedKeyArrays.length; i++) {
        ...
        for (let j = 0; j < arrLockedKeys.length; j++) {
            if (arrKeys.indexOf(arrLockedKeys[j]) !== -1) { return true; }
        }
    }
    return false;
}
...
function exec(arrKeys, proc, next_proc) {
    ...
    let bLocked = true;
    proc(function () {
        if (!bLocked) { throw Error('double unlock?'); }
        bLocked = false;
        release(arrKeys);
        console.log('lock released', arrKeys);
        if (next_proc) { next_proc.apply(next_proc, arguments); }
        handleQueue();
    });
}
...
function handleQueue() {
    ...
    for (let i = 0; i < arrQueuedJobs.length; i++) {
        ...
        if (isAnyOfKeysLocked(job.arrKeys)) { continue; }
...
function checkForDeadlocks() {
    for (let i = 0; i < arrQueuedJobs.length; i++) {
        const job = arrQueuedJobs[i];
        if (Date.now() - job.ts > 30 * 1000) { throw Error(`possible deadlock on job ${require('util').inspect(job)},\nproc:${job.proc.toString()} \nall jobs: ${require('util').inspect(arrQueuedJobs, { depth: null })}`); }
    }
}
...
setInterval(() => {
    console.log(`queued jobs: ${JSON.stringify(arrQueuedJobs.map(job => job.arrKeys))}, locked keys: ${JSON.stringify(arrLockedKeyArrays)}`);
}, 10000);
