...
function mark_start(tag, id) {
    ...
    if (timers[tag][id]) { throw Error(`multiple start marks for ${tag}[${id}]`); }
...
function mark_end(tag, id) {
    ...
    if (!id) id = 0;
    if (!timers_results[tag]) { timers_results[tag] = []; }
    timers_results[tag].push(Date.now() - timers[tag][id]);
    timers[tag][id] = 0;
}

function start() {
    if (start_ts) { throw Error('profiler already started'); }
    start_ts = Date.now();
}

function stop(tag) {
    if (!start_ts) { throw Error('profiler not started'); }
    if (!times[tag]) { times[tag] = 0; }
...
function print() {
    console.log('\nProfiling results:');
    let total = 0;
    for (var tag in times) { total += times[tag]; }
