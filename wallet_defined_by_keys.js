...
function loadBitcoreFromNearestParent(mod) {
  if (!mod) { throw Error('reached root but bitcore not found'); }
  try {
...
function handleOfferToCreateNewWallet(body, from_address, callbacks) {
  ...
  if (!ValidationUtils.isNonemptyString(body.wallet_name)) { return callbacks.ifError('no wallet_name'); }
  if (body.wallet.length > constants.HASH_LENGTH) { return callbacks.ifError('wallet too long'); }
  if (body.wallet_name.length > 200) { return callbacks.ifError('wallet_name too long'); }
  if (!ValidationUtils.isArrayOfLength(body.wallet_definition_template, 2)) { return callbacks.ifError('no definition template'); }
  if (!ValidationUtils.isNonemptyArray(body.other_cosigners)) { return callbacks.ifError('no other_cosigners'); }
  ...
  validateWalletDefinitionTemplate(body.wallet_definition_template, from_address, (err, arrDeviceAddresses) => {
    ...
    if (body.other_cosigners.length !== arrDeviceAddresses.length - 1) { return callbacks.ifError('wrong length of other_cosigners'); }
    const arrOtherDeviceAddresses = _.uniq(body.other_cosigners.map(cosigner => cosigner.device_address));
    arrOtherDeviceAddresses.push(from_address);
    if (!_.isEqual(arrDeviceAddresses.sort(), arrOtherDeviceAddresses.sort())) { return callbacks.ifError('wrong other_cosigners'); }
    for (let i = 0; i < body.other_cosigners.length; i++) {
      const cosigner = body.other_cosigners[i];
      if (!ValidationUtils.isStringOfLength(cosigner.pubkey, constants.PUBKEY_LENGTH)) { return callbacks.ifError('bad pubkey'); }
      if (cosigner.device_address !== objectHash.getDeviceAddress(cosigner.pubkey)) { return callbacks.ifError('bad cosigner device address'); }
      if (!ValidationUtils.isNonemptyString(cosigner.name)) { return callbacks.ifError('no cosigner name'); }
      if (cosigner.name.length > 100) { return callbacks.ifError('cosigner name too long'); }
      if (!ValidationUtils.isNonemptyString(cosigner.hub)) { return callbacks.ifError('no cosigner hub'); }
      if (cosigner.hub.length > 100) { return callbacks.ifError('cosigner hub too long'); }
    }
...
function checkAndFinalizeWallet(wallet, onDone) {
  db.query('SELECT member_ready_date FROM wallets LEFT JOIN extended_pubkeys USING(wallet) WHERE wallets.wallet=?', [wallet], (rows) => {
    ...
    if (rows.some(row => !row.member_ready_date)) { return onDone ? onDone() : null; }
...
function checkAndFullyApproveWallet(wallet, onDone) {
  db.query('SELECT approval_date FROM wallets LEFT JOIN extended_pubkeys USING(wallet) WHERE wallets.wallet=?', [wallet], (rows) => {
    if (rows.length === 0) // wallet not created yet
      { return onDone ? onDone() : null; }
    if (rows.some(row => !row.approval_date)) { return onDone ? onDone() : null; }
...
function createWallet(xPubKey, account, arrWalletDefinitionTemplate, walletName, isSingleAddress, handleWallet) {
  ...
  addWallet(wallet, xPubKey, account, arrWalletDefinitionTemplate, () => {
    ...
    device.readCorrespondentsByDeviceAddresses(arrDeviceAddresses, (arrOtherCosigners) => {
      if (arrOtherCosigners.length !== arrDeviceAddresses.length - 1) { throw Error('incorrect length of other cosigners'); }
      arrDeviceAddresses.forEach((device_address) => {
        if (device_address === device.getMyDeviceAddress()) { return; }
...
function createMultisigWallet(xPubKey, account, count_required_signatures, arrDeviceAddresses, walletName, isSingleAddress, handleWallet) {
  if (count_required_signatures > arrDeviceAddresses.length) { throw Error('required > length'); }
...
function createWalletByDevices(xPubKey, account, count_required_signatures, arrOtherDeviceAddresses, walletName, isSingleAddress, handleWallet) {
  ...
  if (arrOtherDeviceAddresses.length === 0) { createSinglesigWallet(xPubKey, account, walletName, handleWallet); } else {
...
function approveWallet(wallet, xPubKey, account, arrWalletDefinitionTemplate, arrOtherCosigners, onDone) {
  ...
  device.addIndirectCorrespondents(arrOtherCosigners, () => {
    addWallet(wallet, xPubKey, account, arrWalletDefinitionTemplate, () => {
      arrDeviceAddresses.forEach((device_address) => {
        if (device_address !== device.getMyDeviceAddress()) { sendMyXPubKey(device_address, wallet, xPubKey); }
      });
      if (onDone) { onDone(); }
    });
  });
...
function cancelWallet(wallet, arrDeviceAddresses, arrOtherCosigners) {
  ...
  const arrOtherDeviceAddresses = _.uniq(arrOtherCosigners.map(cosigner => cosigner.device_address));
  const arrInitiatorDeviceAddresses = _.difference(arrDeviceAddresses, arrOtherDeviceAddresses);
  if (arrInitiatorDeviceAddresses.length !== 1) { throw Error('not one initiator?'); }
  const initiator_device_address = arrInitiatorDeviceAddresses[0];
  sendCommandToCancelNewWallet(initiator_device_address, wallet);
  arrOtherCosigners.forEach((cosigner) => {
    if (cosigner.device_address === device.getMyDeviceAddress()) { return; }
    // can't use device.sendMessageToDevice because some of the proposed cosigners might not be paired
    device.sendMessageToHub(cosigner.hub, cosigner.pubkey, 'cancel_new_wallet', { wallet });
  });
...
function deleteWallet(wallet, rejector_device_address, onDone) {
  db.query('SELECT approval_date FROM extended_pubkeys WHERE wallet=? AND device_address=?', [wallet, rejector_device_address], (rows) => {
    ...
    db.query('SELECT device_address FROM extended_pubkeys WHERE wallet=?', [wallet], (rows) => {
      const arrMemberAddresses = rows.map(row => row.device_address);
      const arrQueries = [];
    ...
    (rows) => {
      rows.forEach((row) => {
        if (row.device_address === device.getMyDeviceAddress()) {
          if (row.name !== null) { throw Error('found self in correspondents'); }
          row.me = true;
        } else if (row.name === null) { throw Error(`cosigner not found among correspondents, cosigner=${row.device_address}, my=${device.getMyDeviceAddress()}`); }
      });
...
function addNewAddress(wallet, is_change, address_index, address, handleError) {
  ...
  db.query('SELECT 1 FROM wallets WHERE wallet=?', [wallet], (rows) => {
    if (rows.length === 0) { return handleError(`wallet ${wallet} does not exist`); }
    deriveAddress(wallet, is_change, address_index, (new_address, arrDefinition) => {
      if (new_address !== address) { return handleError(`I derived address ${new_address}, your address ${address}`); }
...
function getDeviceAddressesBySigningPaths(arrWalletDefinitionTemplate) {
  function evaluate(arr, path) {
    ...
    switch (op) {
      case 'sig':
        if (!args.pubkey || args.pubkey.substr(0, prefix.length) !== prefix) { return; }
        var device_address = args.pubkey.substr(prefix.length);
        assocDeviceAddressesBySigningPaths[path] = device_address;
        break;
      case 'hash':
        if (!args.hash || args.hash.substr(0, prefix.length) !== prefix) { return; }
        var device_address = args.hash.substr(prefix.length);
        assocDeviceAddressesBySigningPaths[path] = device_address;
        break;
      case 'or':
      case 'and':
        for (var i = 0; i < args.length; i++) { evaluate(args[i], `${path}.${i}`); }
        break;
      case 'r of set':
        if (!ValidationUtils.isNonemptyArray(args.set)) { return; }
        for (var i = 0; i < args.set.length; i++) { evaluate(args.set[i], `${path}.${i}`); }
        break;
      case 'weighted and':
        if (!ValidationUtils.isNonemptyArray(args.set)) { return; }
        for (var i = 0; i < args.set.length; i++) { evaluate(args.set[i].value, `${path}.${i}`); }
        break;
      case 'address':
...
function validateWalletDefinitionTemplate(arrWalletDefinitionTemplate, from_address, handleResult) {
  const arrDeviceAddresses = getDeviceAddresses(arrWalletDefinitionTemplate);
  if (arrDeviceAddresses.indexOf(device.getMyDeviceAddress()) === -1) { return handleResult('my device address not mentioned in the definition'); }
  if (arrDeviceAddresses.indexOf(from_address) === -1) { return handleResult('sender device address not mentioned in the definition'); }
...
function deriveAddress(wallet, is_change, address_index, handleNewAddress) {
  db.query('SELECT definition_template, full_approval_date FROM wallets WHERE wallet=?', [wallet], (wallet_rows) => {
    if (wallet_rows.length === 0) { throw Error(`wallet not found: ${wallet}, is_change=${is_change}, index=${address_index}`); }
    if (!wallet_rows[0].full_approval_date) { throw Error(`wallet not fully approved yet: ${wallet}`); }
    ...
    db.query(
      ...
      [wallet],
      (rows) => {
        if (rows.length === 0) { throw Error(`no extended pubkeys in wallet ${wallet}`); }
        const path = `m/${is_change}/${address_index}`;
        const params = {};
        rows.forEach((row) => {
          if (!row.extended_pubkey) { throw Error(`no extended_pubkey for wallet ${wallet}`); }
...
function recordAddress(wallet, is_change, address_index, address, arrDefinition, onDone) {
  if (typeof address_index === 'string' && is_change) { throw Error('address with string index cannot be change address'); }
  ...
  db.query( // IGNORE in case the address was already generated
    ...
    () => {
      ...
      if (conf.bLight && !is_change) { network.addLightWatchedAddress(address); }
    }
  );
}
...
function issueAddress(wallet, is_change, address_index, handleNewAddress) {
  ...
  deriveAndRecordAddress(wallet, is_change, address_index, (address) => {
    db.query('SELECT device_address FROM extended_pubkeys WHERE wallet=?', [wallet], (rows) => {
      rows.forEach((row) => {
        if (row.device_address !== device.getMyDeviceAddress()) { sendNewWalletAddress(row.device_address, wallet, is_change, address_index, address); }
      });
      handleNewAddress({
        address,
        is_change,
...
function selectRandomAddress(wallet, is_change, from_index, handleAddress) {
  if (from_index === null) { from_index = -1; }
  db.query(
    `SELECT address, address_index, ${db.getUnixTimestamp('creation_date')} AS creation_ts \n\
...
function issueOrSelectNextAddress(wallet, is_change, handleAddress) {
  readNextAddressIndex(wallet, is_change, (next_index) => {
    if (next_index < MAX_BIP44_GAP) { return issueAddress(wallet, is_change, next_index, handleAddress); }
    readLastUsedAddressIndex(wallet, is_change, (last_used_index) => {
      if (last_used_index === null || next_index - last_used_index >= MAX_BIP44_GAP) { selectRandomAddress(wallet, is_change, last_used_index, handleAddress); } else { issueAddress(wallet, is_change, next_index, handleAddress); }
    });
  });
}
...
function issueOrSelectNextChangeAddress(wallet, handleAddress) {
  readNextAddressIndex(wallet, 1, (next_index) => {
    readLastUsedAddressIndex(wallet, 1, (last_used_index) => {
      ...
      if (first_unused_index > next_index) { throw Error('unued > next'); }
      if (first_unused_index < next_index) { readAddressByIndex(wallet, 1, first_unused_index, handleAddress); } else { issueAddress(wallet, 1, next_index, handleAddress); }
    });
  });
}
...
function issueOrSelectAddressForApp(wallet, app_name, handleAddress) {
  db.query('SELECT address FROM my_addresses WHERE wallet=? AND app=?', [wallet, app_name], (rows) => {
    if (rows.length > 1) { throw Error(`more than 1 address for app ${app_name}`); }
    if (rows.length === 1) { return handleAddress(rows[0].address); }
    issueAddress(wallet, 0, app_name, (addressInfo) => {
...
function checkAddress(account, is_change, address_index) {
  db.query('SELECT wallet, extended_pubkey FROM wallets JOIN extended_pubkeys USING(wallet) WHERE account=?', [account], (rows) => {
    ...
    db.query(
      ...
      (address_rows) => {
        if (address_rows.length === 0) { return; }
        ...
        if (address_row.address !== address) { throw Error(`address mismatch, derived: ${address}, db: ${address_row.address}`); }
...
function readAddresses(wallet, opts, handleAddresses) {
  ...
  if (opts.is_change === 0 || opts.is_change === 1) { sql += ` AND is_change=${opts.is_change}`; }
  sql += ' ORDER BY creation_ts';
  if (opts.reverse) { sql += ' DESC'; }
...
function readAddressInfo(address, handleAddress) {
  db.query('SELECT address_index, is_change FROM my_addresses WHERE address=?', [address], (rows) => {
    if (rows.length === 0) { return handleAddress(`address ${address} not found`); }
...
function readAllAddresses(wallet, handleAddresses) {
  db.query(
    ...
    [wallet],
    (rows) => {
      handleAddresses(rows.map(row => row.address));
    }
  );
}
...
function forwardPrivateChainsToOtherMembersOfWallets(arrChains, arrWallets, conn, onSaved) {
  ...
  conn.query(
    ...
    (rows) => {
      const arrDeviceAddresses = rows.map(row => row.device_address);
...
function readDeviceAddressesControllingPaymentAddresses(conn, arrAddresses, handleDeviceAddresses) {
  if (arrAddresses.length === 0) { return handleDeviceAddresses([]); }
  conn = conn || db;
  conn.query(
