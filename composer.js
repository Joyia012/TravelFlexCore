...
function pickDivisibleCoinsForAmount(conn, objAsset, arrAddresses, last_ball_mci, amount, bMultiAuthored, onDone) {
    ...
    function addInput(input) {
        ...
        if (objAsset && objAsset.is_private) { // for type=payment only
            ...
            if (bMultiAuthored) { objSpendProof.address = input.address; }
            objInputWithProof.spend_proof = objSpendProof;
        }
        if (!bMultiAuthored || !input.type) { delete input.address; }
        delete input.amount;
        ...
        conn.query(
            ...
            (rows) => {
                async.eachSeries(
                    ...
                    (err) => {
                        if (err === 'found') { onDone(arrInputsWithProofs, total_amount); } else if (asset) { issueAsset(); } else { addHeadersCommissionInputs(); }
                    }
                );
            }
        );
    ...
    function addMcInputs(type, input_size, max_mci, onStillNotEnough) {
        async.eachSeries(
            ...
            (address, cb) => {
                ...
                mc_outputs.findMcIndexIntervalToTargetAmount(conn, type, address, max_mci, target_amount, {
                    ...
                    ifFound(from_mc_index, to_mc_index, earnings, bSufficient) {
                        if (earnings === 0) { throw Error('earnings === 0'); }
            ...
            (err) => {
                if (!err) { console.log(`${arrAddresses} ${type}: got only ${total_amount} out of required ${required_amount}`); }
    ...
    function issueAsset() {
        ...
        if (objAsset.issued_by_definer_only && arrAddresses.indexOf(objAsset.definer_address) === -1) { return finish(); }
        ...
        if (objAsset.cap) {
            conn.query("SELECT 1 FROM inputs WHERE type='issue' AND asset=?", [asset], (rows) => {
    ...
    function finish() {
        if (amount === Infinity && arrInputsWithProofs.length > 0) { onDone(arrInputsWithProofs, total_amount); } else { onDone(null); }
    }
    ...
    if (arrSpendableAddresses.length > 0) { pickOneCoinJustBiggerAndContinue(); } else { issueAsset(); }
}
...
function composeJoint(params) {
    ...
    if (params.minimal && !params.send_all) {
        ...
        var trySubset = function (count) {
            if (count > constants.MAX_AUTHORS_PER_UNIT) { return callbacks.ifNotEnoughFunds('Too many authors.  Consider splitting the payment into two units.'); }
            ...
            try_params.callbacks = {
                ...
                ifNotEnoughFunds(error_message) {
                    if (count === arrCandidatePayingAddresses.length) { return callbacks.ifNotEnoughFunds(error_message); }
    ...
    if (params.earned_headers_commission_recipients) // it needn't be already sorted by address, we'll sort it now
        {
 objUnit.earned_headers_commission_recipients = params.earned_headers_commission_recipients.concat().sort((a, b) => ((a.address < b.address) ? -1 : 1));
    ...
    async.series([
        ...
        function (cb) { // lightProps
            if (!conf.bLight) { return cb(); }
            const network = require('./network.js');
            network.requestFromLightVendor(
                'light/get_parents_and_last_ball_and_witness_list_unit',
                { witnesses: arrWitnesses },
                (ws, request, response) => {
                    if (response.error) { return handleError(response.error); } // cb is not called
                    if (!response.parent_units || !response.last_stable_mc_ball || !response.last_stable_mc_ball_unit || typeof response.last_stable_mc_ball_mci !== 'number') { return handleError('invalid parents from light vendor'); } // cb is not called
                    lightProps = response;
                    cb();
                }
            );
        ...
        function (cb) { // authors
            async.eachSeries(arrFromAddresses, (from_address, cb2) => {
                ...
                signer.readSigningPaths(conn, from_address, (assocLengthsBySigningPaths) => {
                    ...
                    for (let j = 0; j < arrSigningPaths.length; j++) { objAuthor.authentifiers[arrSigningPaths[j]] = repeatString('-', assocLengthsBySigningPaths[arrSigningPaths[j]]); }
        ...
        function (cb) { // witnesses
            ...
            if (conf.bLight) {
                if (lightProps.witness_list_unit) { objUnit.witness_list_unit = lightProps.witness_list_unit; } else { objUnit.witnesses = arrWitnesses; }
                return cb();
            }
            ...
            storage.determineIfWitnessAddressDefinitionsHaveReferences(conn, arrWitnesses, (bWithReferences) => {
                if (bWithReferences) { return cb('some witnesses have references in their addresses'); }
                storage.findWitnessListUnit(conn, arrWitnesses, last_ball_mci, (witness_list_unit) => {
                    if (witness_list_unit) { objUnit.witness_list_unit = witness_list_unit; } else { objUnit.witnesses = arrWitnesses; }
                    cb();
                });
            });
        },
 ...
 return cb({
                    ...
                    objPaymentMessage.payload.inputs = arrInputsWithProofs.map(objInputWithProof => objInputWithProof.input);
                    objUnit.payload_commission = objectLength.getTotalPayloadSize(objUnit);
        ...
        conn.query(err ? 'ROLLBACK' : 'COMMIT', () => {
            ...
            if (change <= 0) {
                if (!params.send_all) { throw Error(`change=${change}, params=${JSON.stringify(params)}`); }
                return handleError({
                    error_code: 'NOT_ENOUGH_FUNDS',
            ...
            async.each(
                ...
                (err) => {
                    ...
                    if (bGenesis) { objJoint.ball = objectHash.getBallHash(objUnit.unit); }
                    ...
                    if (Object.keys(assocPrivatePayloads).length === 0) { assocPrivatePayloads = null; }
                    // profiler.stop('compose');
...
function filterMostFundedAddresses(rows, estimated_amount) {
    if (!estimated_amount) { return rows.map(row => row.address); }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (accumulated_amount > estimated_amount + MAX_FEE) { break; }
    }
...
function readSortedFundedAddresses(asset, arrAvailableAddresses, estimated_amount, handleFundedAddresses) {
    if (arrAvailableAddresses.length === 0) { return handleFundedAddresses([]); }
...
function composeMinimalJoint(params) {
    const estimated_amount = (params.send_all || params.retrieveMessages) ? 0 : params.outputs.reduce((acc, output) => acc + output.amount, 0) + TYPICAL_FEE;
    readSortedFundedAddresses(null, params.available_paying_addresses, estimated_amount, (arrFundedPayingAddresses) => {
        if (arrFundedPayingAddresses.length === 0) { return params.callbacks.ifNotEnoughFunds('all paying addresses are unfunded'); }
                ...
                ifOk(objValidationState, validation_unlock) {
                    ...
                    postJointToLightVendorIfNecessaryAndSave(
                        ...
                        (err) => { // light only
                            ...
                            if (err.match(/signature/)) { eventBus.emit('nonfatal_error', `failed to post unit ${unit}: ${err}; ${JSON.stringify(objUnit)}`, new Error()); }
                            validation_unlock();
                            composer_unlock();
                            callbacks.ifError(err);
                        },
                        () => {
                            writer.saveJoint(
                                ...
                                (conn, cb) => {
                                    if (typeof callbacks.preCommitCb === 'function') { callbacks.preCommitCb(conn, objJoint, cb); } else { cb(); }
                                },
                                (err) => {
                                    validation_unlock();
                                    composer_unlock();
...
function postJointToLightVendorIfNecessaryAndSave(objJoint, onLightError, save) {
    if (conf.bLight) { // light clients cannot save before receiving OK from light vendor
        ...
        network.postJointToLightVendor(objJoint, (response) => {
            if (response === 'accepted') { save(); } else { onLightError(response.error); }
        });
    } else { save(); }
}
...
if (objUnit.messages[i].payload_hash === payload_hash) { return i; }
}
