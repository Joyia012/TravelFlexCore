...
function calcHeadersCommissions(conn, onDone) {
    ...
    if (max_spendable_mci === null) // first calc after restart only
        { return initMaxSpendableMci(conn, () => { calcHeadersCommissions(conn, onDone); }); }

    // max_spendable_mci is old, it was last updated after previous calc
    const since_mc_index = max_spendable_mci;
    ...
    async.series([
        function (cb) {
            if (conf.storage === 'mysql') {
                ...
                conn.query(
                    ...
                    [since_mc_index],
                    (rows) => {
                        const assocChildrenInfos = {};
                        rows.forEach((row) => {
                            const payer_unit = row.payer_unit;
                            const child_unit = row.child_unit;
                            if (!assocChildrenInfos[payer_unit]) { assocChildrenInfos[payer_unit] = { headers_commission: row.headers_commission, children: [] }; } else if (assocChildrenInfos[payer_unit].headers_commission !== row.headers_commission) { throw Error('different headers_commission'); }
                        ...
                        for (const payer_unit in assocChildrenInfos) {
                            ...
                            if (!assocWonAmounts[child_unit]) { assocWonAmounts[child_unit] = {}; }
                            assocWonAmounts[child_unit][payer_unit] = headers_commission;
                        }
                        // console.log(assocWonAmounts);
                        const arrWinnerUnits = Object.keys(assocWonAmounts);
                        if (arrWinnerUnits.length === 0) { return cb(); }
                        const strWinnerUnitsList = arrWinnerUnits.map(db.escape).join(', ');
                        conn.query(
                            `SELECT \n\
                                unit_authors.unit, \n\
                            ...
                            (profit_distribution_rows) => {
                                ...
                                conn.query(`INSERT INTO headers_commission_contributions (unit, address, amount) VALUES ${arrValues.join(', ')}`, () => {
                                    cb();
                                });
                            }
                        );
        ...
        function (cb) {
            conn.query('SELECT MAX(main_chain_index) AS max_spendable_mci FROM headers_commission_outputs', (rows) => {
