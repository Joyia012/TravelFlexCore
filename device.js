...
function loginMessage(challenge, priv, pubkey) {
    const objLogin = { challenge, pubkey };
...
function sendTempPubkey(ws, temp_pubkey, callbacks) {
    if (!callbacks)   { callbacks = { ifOk() {}, ifError() {} }; }
...
function sendPreparedMessageToHub(ws, recipient_device_pubkey, message_hash, json, callbacks) {
    ...
    if (typeof ws === 'string') {
        const hub_host = ws;
        network.findOutboundPeerOrConnect(conf.WS_PROTOCOL + hub_host, (err, ws) => {
            if (err) {
...
function sendPreparedMessageToConnectedHub(ws, recipient_device_pubkey, message_hash, json, callbacks) {
    ...
    } else        { handleError(response.error || (`unrecognized response: ${JSON.stringify(response)}`)); }
});
});
}
...
function addIndirectCorrespondents(arrOtherCosigners, onDone) {
    async.eachSeries(arrOtherCosigners, (correspondent, cb) => {
...
function requestFromHub(command, params, responseHandler) {
    if (!my_device_hub)   { return setTimeout(() => { requestFromHub(command, params, responseHandler); }, 2000); }
    network.findOutboundPeerOrConnect(conf.WS_PROTOCOL + my_device_hub, (err, ws) => {
