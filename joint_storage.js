...
function checkIfNewUnit(unit, callbacks) {
    if (storage.isKnownUnit(unit)) { return callbacks.ifKnown(); }
    db.query('SELECT 1 FROM units WHERE unit=?', [unit], (rows) => {
        if (rows.length > 0) {
            storage.setUnitIsKnown(unit);
            return callbacks.ifKnown();
        }
        db.query('SELECT 1 FROM unhandled_joints WHERE unit=?', [unit], (unhandled_rows) => {
            if (unhandled_rows.length > 0) { return callbacks.ifKnownUnverified(); }
    ...
    mutex.lock(['dependencies'], (unlock) => {
        db.query(
            ...
            (rows) => {
                ...
                rows.forEach((row) => {
                    db.query(`SELECT json, peer, ${db.getUnixTimestamp('creation_date')} AS creation_ts FROM unhandled_joints WHERE unit=?`, [row.unit_for_json], (internal_rows) => {
                        internal_rows.forEach((internal_row) => {
...
function findLostJoints(handleLostJoints) {
    ...
    db.query(
        ...
        (rows) => {
            ...
            handleLostJoints(rows.map(row => row.depends_on_unit));
        }
    );
}
...
function collectQueriesToPurgeDependentJoints(conn, arrQueries, unit, error, onPurgedDependentJoint, onDone) {
    conn.query('SELECT unit, peer FROM dependencies JOIN unhandled_joints USING(unit) WHERE depends_on_unit=?', [unit], (rows) => {
        ...
        async.eachSeries(
            rows,
            (row, cb) => {
                if (onPurgedDependentJoint) { onPurgedDependentJoint(row.unit, row.peer); }
...
function purgeUncoveredNonserialJoints(bByExistenceOfChildren, onDone) {
    ...
    (rows) => {
      async.eachSeries(
                ...
                mutex.lock(['write'], (unlock) => {
                  ...
                  archiving.generateQueriesToArchiveJoint(conn, objJoint, 'uncovered', arrQueries, () => {
                    ...
                    async.series(arrQueries, () => {
                      ...
                      const parent_units = storage.assocUnstableUnits[row.unit].parent_units;
                      storage.forgetUnit(row.unit);
                      parent_units.forEach((parent_unit) => {
                        ...
                        if (!bHasChildren) { storage.assocUnstableUnits[parent_unit].is_free = 1; }
                      });
                      cb();
                    });
                  });
        ...
        () => {
          if (rows.length > 0) { return purgeUncoveredNonserialJoints(true, onDone); } // to clean chains of bad units
          if (!bByExistenceOfChildren) { return onDone(); }
