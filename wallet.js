  ...
  switch (subject) {
    ...
    case 'hub/message':
      ...
      if (!message_hash || !objDeviceMessage || !objDeviceMessage.signature || !objDeviceMessage.pubkey || !objDeviceMessage.to
        ...
        || !objDeviceMessage.encrypted_package.iv || !objDeviceMessage.encrypted_package.authtag) { return network.sendError(ws, 'missing fields'); }
  ...
  switch (subject) {
    ...
    case 'approve_new_shared_address':
      ...
      if (typeof body.device_addresses_by_relative_signing_paths !== 'object'
        || Object.keys(body.device_addresses_by_relative_signing_paths).length === 0) { return callbacks.ifError('invalid device_addresses_by_relative_signing_paths'); }
...
hidden_payload.outputs.forEach((o) => {
          ...
          if (objUnit.messages.filter(objMessage => (objMessage.payload_hash === payload_hash)).length !== 1) { return callbacks.ifError('no such payload hash in the messages'); }
        }
      }
...
if (!assocValidatedByKey[key]) { return console.log('not all private payments validated yet'); }
        ...
        if (!body.forwarded) {
          ...
          db.query('SELECT address FROM unit_authors WHERE unit IN(?)', [arrUnits], (rows) => {
            const arrAuthorAddresses = rows.map(row => row.address);
...
function forwardPrivateChainsToOtherMembersOfOutputAddresses(arrChains, conn, onSaved) {
  ...
  arrChains.forEach((arrPrivateElements) => {
    ...
    payload.outputs.forEach((output) => {
      if (output.address) { assocOutputAddresses[output.address] = true; }
    });
    if (objHeadPrivateElement.output && objHeadPrivateElement.output.address) { assocOutputAddresses[objHeadPrivateElement.output.address] = true; }
  });
...
function readWalletsByAddresses(conn, arrAddresses, handleWallets) {
  conn.query('SELECT DISTINCT wallet FROM my_addresses WHERE address IN(?)', [arrAddresses], (rows) => {
    const arrWallets = rows.map(row => row.wallet);
    conn.query('SELECT DISTINCT address FROM shared_address_signing_paths WHERE shared_address IN(?)', [arrAddresses], (rows) => {
      if (rows.length === 0) { return handleWallets(arrWallets); }
      const arrNewAddresses = rows.map(row => row.address);
...
function emitNewPrivatePaymentReceived(payer_device_address, arrChains, message_counter) {
  ...
  walletGeneral.readMyAddresses((arrAddresses) => {
    ...
    arrChains.forEach((arrPrivateElements) => {
      ...
      if (!assocAmountsByAsset[asset]) { assocAmountsByAsset[asset] = 0; }
      payload.outputs.forEach((output) => {
...
function emitNewPublicPaymentReceived(payer_device_address, objUnit, message_counter) {
  walletGeneral.readMyAddresses((arrAddresses) => {
    ...
    objUnit.messages.forEach((message) => {
      if (message.app !== 'payment' || !message.payload) { return; }
...
function readSharedBalance(wallet, handleBalance) {
  balances.readSharedBalance(wallet, (assocBalances) => {
...
function readAssetMetadata(arrAssets, handleMetadata) {
  ...
  db.query(sql, (rows) => {
    ...
    if (!arrAssets) { return; }
    const updateAssets = conf.bLight ? network.requestProofsOfJointsIfNewOrUnstable : function (arrAssets, onDone) { onDone(); };
    updateAssets(arrAssets, () => { // make sure we have assets itself
      arrAssets.forEach((asset) => {
        ...
        fetchAssetMetadata(asset, (err, objMetadata) => {
...
function readTransactionHistory(opts, handleHistory) {
  ...
  if (opts.wallet && opts.address || !opts.wallet && !opts.address) { throw Error('invalid wallet and address params'); }
  ...
  let cross = '';
  if (opts.unit) { where_condition += ` AND unit=${db.escape(opts.unit)}`; } else if (opts.since_mci && ValidationUtils.isNonnegativeInteger(opts.since_mci)) {
 ...
 assocMovements[row.unit] = {
        ...
        if (row.to_address) {
          ...
          if (!assocMovements[row.unit].arrMyRecipients) { assocMovements[row.unit].arrMyRecipients = []; }
          assocMovements[row.unit].arrMyRecipients.push({ my_address: row.to_address, amount: row.amount });
        }
        if (row.from_address) { assocMovements[row.unit].has_minus = true; }
      }
        ...
        (movement, unit, cb) => {
          if (movement.sequence !== 'good') {
            ...
            db.query(
              ...
              [unit],
              (address_rows) => {
                const arrPayerAddresses = address_rows.map(address_row => address_row.address);
                movement.arrMyRecipients.forEach((objRecipient) => {
                  const transaction = {
                    action: 'received',
...
parameters;
            ...
            db.query(queryString, parameters,
              (payee_rows) => {
                const action = payee_rows.some(payee => payee.is_external) ? 'sent' : 'moved';
                ...
                async.eachSeries(payee_rows, (payee, cb2) => {
                  ...
                  const transaction = {
                    ...
                    level: movement.level,
                    mci: movement.mci,
                    isTextcoin: !!payee.textAddress
                  };
                  if (action === 'moved') { transaction.my_address = payee.address; }
...
function determineIfFixedDenominations(asset, handleResult) {
  if (!asset) { return handleResult(false); }
  storage.readAsset(db, asset, null, (err, objAsset) => {
...
function readFundedAddresses(asset, wallet, estimated_amount, handleFundedAddresses) {
  ...
  if (walletIsAddresses) { return composer.readSortedFundedAddresses(asset, wallet, estimated_amount, handleFundedAddresses); }
  if (estimated_amount && typeof estimated_amount !== 'number') { throw Error(`invalid estimated amount: ${estimated_amount}`); }
  ...
  db.query(
    ...
    (rows) => {
      determineIfFixedDenominations(asset, (bFixedDenominations) => {
        if (bFixedDenominations) { estimated_amount = 0; } // don't shorten the list of addresses, indivisible_asset.js will do it later according to denominations
...
function readAdditionalSigningAddresses(arrPayingAddresses, arrSigningAddresses, arrSigningDeviceAddresses, handleAdditionalSigningAddresses) {
  ...
  db.query(
    ...
    arrParams,
    (rows) => {
      const arrAdditionalAddresses = rows.map(row => row.address);
      if (arrAdditionalAddresses.length === 0) { return handleAdditionalSigningAddresses([]); }
...
function readFundedAndSigningAddresses(asset, wallet, estimated_amount, fee_paying_wallet, arrSigningAddresses, arrSigningDeviceAddresses, handleFundedAndSigningAddresses) {
  readFundedAddresses(asset, wallet, estimated_amount, (arrFundedAddresses) => {
    if (arrFundedAddresses.length === 0) { return handleFundedAndSigningAddresses([], [], []); }
    let arrBaseFundedAddresses = [];
    const addSigningAddressesAndReturn = function () {
    ...
    if (!asset) { return addSigningAddressesAndReturn(); }
...
function sendMultiPayment(opts, handleResult) {
  ...
  if (!wallet && !arrPayingAddresses) { throw Error('neither wallet id nor paying addresses'); }
  if (wallet && arrPayingAddresses) { throw Error('both wallet id and paying addresses'); }
  if ((to_address || amount) && (base_outputs || asset_outputs)) { throw Error('to_address and outputs at the same time'); }
  if (!asset && asset_outputs) { throw Error('base asset and asset outputs'); }
  if (amount) {
    if (typeof amount !== 'number') { throw Error('amount must be a number'); }
    if (amount < 0) { throw Error('amount must be positive'); }
  }

  if (recipient_device_address === device.getMyDeviceAddress()) { recipient_device_address = null; }
...
estimated_amount = asset_outputs.reduce((acc, output) => acc + output.amount, 0);
  ...
  if (estimated_amount && !asset) { estimated_amount += TYPICAL_FEE; }
  ...
  readFundedAndSigningAddresses(
    ...
    (arrFundedAddresses, arrBaseFundedAddresses, arrAllSigningAddresses) => {
      if (arrFundedAddresses.length === 0) { return handleResult('There are no funded addresses'); }
      if (asset && arrBaseFundedAddresses.length === 0) { return handleResult('No bytes to pay fees'); }
      ...
      function generateNewMnemonicIfNoAddress(output_asset, outputs) {
        ...
        outputs.forEach((output) => {
          if (output.address.indexOf(prefix) !== 0) { return false; }
          ...
          assocAddresses[output.address] = objectHash.getChash160(['sig', { pubkey }]);
      ...
      const params = {
        ...
        callbacks: {
          ...
          ifOk(objJoint, arrChainsOfRecipientPrivateElements, arrChainsOfCosignerPrivateElements) {
      ...
      const addFeesToParams = function (objAsset) {
        ...
        for (let orig_address in assocAddresses) {
          ...
          const _addAssetFees = function () {
            ...
            const base_output = _.find(params.base_outputs, output => output.address == new_address);
            if (base_output) { base_output.amount += asset_fees; } else { params.base_outputs.push({ address: new_address, amount: asset_fees }); }
          };

          // first calculate fees for textcoins in (bytes) outputs
          let output = _.find(params.outputs, output => output.address == new_address);
          if (output) {
            output.amount += constants.TEXTCOIN_CLAIM_FEE;
          }

          // second calculate fees for textcoins in base_outputs
          output = _.find(params.base_outputs, output => output.address == new_address);
          if (output) {
            output.amount += constants.TEXTCOIN_CLAIM_FEE;
          }

          // then check for textcoins in asset_outputs
          output = _.find(params.asset_outputs, output => output.address == new_address);
          if (output) {
            _addAssetFees();
          }
          ...
          if (new_address == params.to_address) {
            if (objAsset) {
              ...
              params.asset_outputs = [{ address: new_address, amount }];
              _addAssetFees();
            } else {
      ...
      if (asset) {
        if (bSendAll) { throw Error('send_all with asset'); }
        params.asset = asset;
        ...
        storage.readAsset(db, asset, null, (err, objAsset) => {
          ...
          if (objAsset.is_private) {
            ...
            params.callbacks.preCommitCb = function (conn, arrChainsOfRecipientPrivateElements, arrChainsOfCosignerPrivateElements, cb) {
              if (!arrChainsOfRecipientPrivateElements || !arrChainsOfCosignerPrivateElements) { throw Error('no private elements'); }
              const sendToRecipients = function (cb2) {
                if (recipient_device_address) { walletGeneral.sendPrivatePayments(recipient_device_address, arrChainsOfRecipientPrivateElements, false, conn, cb2); } else // paying to another wallet on the same device
                  { forwardPrivateChainsToOtherMembersOfOutputAddresses(arrChainsOfRecipientPrivateElements, conn, cb2); }
              };
              const sendToCosigners = function (cb2) {
                if (wallet) { walletDefinedByKeys.forwardPrivateChainsToOtherMembersOfWallets(arrChainsOfCosignerPrivateElements, [wallet], conn, cb2); } else // arrPayingAddresses can be only shared addresses
...
function replaceInTextcoinTemplate(params, handleText) {
  ...
  fs.readFile(`${__dirname}/email_template.html`, 'utf8', (err, template) => {
    if (err) { throw Error(`failed to read template: ${err}`); }
    ...
    const text = `Here is your link to receive ${params.amount} ${params.asset}${params.usd_amount_str}: https://byteball.org/#textcoin?${params.mnemonic}`;
...
function receiveTextCoin(mnemonic, addressTo, cb) {
  ...
  opts.callbacks = {
    ...
    ifError(err) {
      if (err.indexOf('some definition changes') == 0) { return cb('This textcoin was already claimed but not confirmed yet'); }
      cb(err);
    },
    ifOk(objJoint, arrChainsOfRecipientPrivateElements, arrChainsOfCosignerPrivateElements) {
      network.broadcastJoint(objJoint);
    ...
    db.query(
      ...
      (rows) => {
        if (rows.length === 0) {
          ...
          } else if (row.asset) { // claiming asset
              opts.asset = row.asset;
              opts.amount = row.amount;
              opts.fee_paying_addresses = [address];
              storage.readAsset(db, row.asset, null, (err, objAsset) => {
  ...
  db.query(
    ...
    (rows) => {
      async.eachSeries(
        rows,
        (row, cb) => {
          receiveTextCoin(row.mnemonic, to_address, (err, unit, asset) => {
            if (err) { console.log(`failed claiming back old textcoin ${row.mnemonic}: ${err}`); } else { console.log(`claimed back mnemonic ${row.mnemonic}, unit ${unit}, asset ${asset}`); }
            cb();
          });
        }
      );
...
function readDeviceAddressesUsedInSigningPaths(onDone) {
  ...
  db.query(
    sql,
    (rows) => {
      const arrDeviceAddress = rows.map(r => r.device_address);
