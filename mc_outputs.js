...
function findMcIndexIntervalToTargetAmount(conn, type, address, max_mci, target_amount, callbacks) {
    ...
    readNextSpendableMcIndex(conn, type, address, null, (from_mci) => {
        if (from_mci > max_mci) { return callbacks.ifNothing(); }
        readMaxSpendableMcIndex(conn, type, (max_spendable_mci) => {
            if (max_spendable_mci <= 0) { return callbacks.ifNothing(); }
            if (max_spendable_mci > max_mci) { max_spendable_mci = max_mci; }
            if (target_amount === Infinity) { target_amount = 1e15; }
            if (conf.storage === 'mysql') {
                conn.query(
                    ...
                    (rows) => {
                        if (rows.length === 0) { return callbacks.ifNothing(); }
...
function calcEarnings(conn, type, from_main_chain_index, to_main_chain_index, address, callbacks) {
    ...
    conn.query(
        ...
        (rows) => {
            ...
            if (typeof total !== 'number') { throw Error('mc outputs total is not a number'); }
