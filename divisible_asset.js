    ...
    validateDivisiblePrivatePayment(conn, objPrivateElement, {
        ...
        ifOk(bStable, arrAuthorAddresses) {
            ...
            for (var j = 0; j < payload.inputs.length; j++) {
                ...
                let address = null,
address_sql = null;
                if (type === 'issue') { address = input.address || arrAuthorAddresses[0]; } else { // transfer
                    if (arrAuthorAddresses.length === 1) { address = arrAuthorAddresses[0]; } else {
...
function validateDivisiblePrivatePayment(conn, objPrivateElement, callbacks) {
    ...
    if (!ValidationUtils.isStringOfLength(payload.asset, constants.HASH_LENGTH)) { return callbacks.ifError('invalid asset in private divisible payment'); }
    if (!ValidationUtils.isNonemptyArray(payload.inputs)) { return callbacks.ifError('no inputs'); }
    ...
    validation.initPrivatePaymentValidationState(
        ...
        (bStable, objPartialUnit, objValidationState) => {
            const arrAuthorAddresses = objPartialUnit.authors.map(author => author.address);
                    ...
                    (input, cb) => {
                        if (input.type === 'issue') {
                            ...
                            conn.query(
                                ...
                                (rows) => {
                                    if (rows.length !== 1) { return cb('not 1 row when selecting src output'); }
                        ...
                        } else { cb(`unknown input type: ${input.type}`); }
                    },
                    (err) => {
                        if (err) { return sp_cb(err); }
                        ...
                        conn.query(
                            ...
                            [unit, message_index],
                            (rows) => {
                                if (rows.length !== arrSpendProofs.length) { return sp_cb('incorrect number of spend proofs'); }
                                for (let i = 0; i < rows.length; i++) {
                                    if (rows[i].address !== arrSpendProofs[i].address || rows[i].spend_proof !== arrSpendProofs[i].spend_proof) { return sp_cb('incorrect spend proof'); }
                                }
                                sp_cb();
                            }
                        );
    ...
    composer.composeJoint({
        ...
        retrieveMessages(conn, last_ball_mci, bMultiAuthored, arrPayingAddresses, onDone) {
            ...
            storage.loadAssetWithListOfAttestedAuthors(conn, params.asset, last_ball_mci, arrAssetPayingAddresses, (err, objAsset) => {
                ...
                if (objAsset.fixed_denominations) { return onDone('fixed denominations asset type'); }
                ...
                const target_amount = params.to_address
                    ? params.amount
                    : params.asset_outputs.reduce((accumulator, output) => accumulator + output.amount, 0);
                composer.pickDivisibleCoinsForAmount(
                    ...
                    (arrInputsWithProofs, total_input) => {
                        ...
                        if (!arrInputsWithProofs) { return onDone({ error_code: 'NOT_ENOUGH_FUNDS', error: 'not enough asset coins' }); }
                        ...
                        if (objAsset.is_private) { arrOutputs.forEach((output) => { output.blinding = composer.generateBlinding(); }); }
                        arrOutputs.sort(composer.sortOutputs);
                        const payload = {
                            asset: params.asset,
                            inputs: arrInputsWithProofs.map(objInputWithProof => objInputWithProof.input),
                            outputs: arrOutputs
                        };
                        const objMessage = {
                            app: 'payment',
                        ...
                        if (objAsset.is_private) {
                            objMessage.spend_proofs = arrInputsWithProofs.map(objInputWithProof => objInputWithProof.spend_proof);
