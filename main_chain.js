...
function updateMainChain(conn, from_unit, last_added_unit, onDone) {
    ...
    function findNextUpMainChainUnit(unit, handleUnit) {
        function handleProps(props) {
            if (props.best_parent_unit === null) { throw Error('best parent is null'); }
        ...
        function readLastUnitProps(handleLastUnitProps) {
            conn.query('SELECT unit AS best_parent_unit, witnessed_level \n\
                ...
                LIMIT 5',
                (rows) => {
                    if (rows.length === 0) { throw Error('no free units?'); }
                    if (rows.length > 1) {
                        const arrParents = rows.map(row => row.best_parent_unit);
                        arrAllParents = arrParents;
                        for (let i = 0; i < arrRetreatingUnits.length; i++) {
                            const n = arrParents.indexOf(arrRetreatingUnits[i]);
                            if (n >= 0) { return handleLastUnitProps(rows[n]); }
                        }
                    }
                    /*
    ...
    function goUpFromUnit(unit) {
        if (storage.isGenesisUnit(unit)) { return checkNotRebuildingStableMainChainAndGoDown(0, unit); }

        profiler.start();
        findNextUpMainChainUnit(unit, (best_parent_unit) => {
            storage.readUnitProps(conn, best_parent_unit, (objBestParentUnitProps) => {
     let objBestParentUnitProps2 = storage.assocUnstableUnits[best_parent_unit];
     if (!objBestParentUnitProps2) {
       if (storage.isGenesisUnit(best_parent_unit)) { objBestParentUnitProps2 = storage.assocStableUnits[best_parent_unit]; } else { throw Error(`unstable unit not found: ${best_parent_unit}`); }
     }
    const objBestParentUnitPropsForCheck = _.cloneDeep(objBestParentUnitProps2);
                ...
                if (!_.isEqual(objBestParentUnitPropsForCheck, objBestParentUnitProps)) { throwError(`different props, db: ${JSON.stringify(objBestParentUnitProps)}, unstable: ${JSON.stringify(objBestParentUnitProps2)}`); }
                if (!objBestParentUnitProps.is_on_main_chain) {
 conn.query('UPDATE units SET is_on_main_chain=1, main_chain_index=NULL WHERE unit=?', [best_parent_unit], () => {
                    ...
                    profiler.stop('mc-goUpFromUnit');
                    if (unit === null) { updateLatestIncludedMcIndex(objBestParentUnitProps.main_chain_index, false); } else { checkNotRebuildingStableMainChainAndGoDown(objBestParentUnitProps.main_chain_index, best_parent_unit); }
                }
            });
        });
    }
    ...
    function checkNotRebuildingStableMainChainAndGoDown(last_main_chain_index, last_main_chain_unit) {
        ...
        conn.query(
            ...
            (rows) => {
                ...
                if (rows.length > 0) { throw Error(`removing stable units ${rows.map(row => row.unit).join(', ')} from MC after adding ${last_added_unit} with all parents ${arrAllParents.join(', ')}`); }
    ...
    function goDownAndUpdateMainChainIndex(last_main_chain_index, last_main_chain_unit) {
        ...
        conn.query(
            ...
            () => {
                ...
                const main_chain_unit = last_main_chain_unit;
                conn.query(
                    ...
                    (rows) => {
                        ...
                        const arrDbNewMcUnits = rows.map(row => row.unit);
                        arrNewMcUnits.reverse();
                        if (!_.isEqual(arrNewMcUnits, arrDbNewMcUnits)) { throwError(`different new MC units, arr: ${JSON.stringify(arrNewMcUnits)}, db: ${JSON.stringify(arrDbNewMcUnits)}`); }
                        async.eachSeries(
                            rows,
                            (row, cb) => {
                                main_chain_index++;
                                ...
                                function goUp(arrStartUnits) {
                                    conn.query(
                                        ...
                                        [arrStartUnits],
                                        (rows) => {
                                            const arrNewStartUnits = rows.map(row => row.unit);
                                            const arrNewStartUnits2 = [];
                                            arrStartUnits.forEach((start_unit) => {
                                                storage.assocUnstableUnits[start_unit].parent_units.forEach((parent_unit) => {
                                                    if (storage.assocUnstableUnits[parent_unit] && storage.assocUnstableUnits[parent_unit].main_chain_index === null && arrNewStartUnits2.indexOf(parent_unit) === -1) { arrNewStartUnits2.push(parent_unit); }
                                                });
                                            });
                                            if (!_.isEqual(arrNewStartUnits.sort(), arrNewStartUnits2.sort())) { throwError(`different new start units, arr: ${JSON.stringify(arrNewStartUnits2)}, db: ${JSON.stringify(arrNewStartUnits)}`); }
                                            if (arrNewStartUnits.length === 0) { return updateMc(); }
                                ...
                                function updateMc() {
                                    ...
                                    const strUnitList = arrUnits.map(db.escape).join(', ');
                                    conn.query(`UPDATE units SET main_chain_index=? WHERE unit IN(${strUnitList})`, [main_chain_index], () => {
                                        conn.query(`UPDATE unit_authors SET _mci=? WHERE unit IN(${strUnitList})`, [main_chain_index], () => {
                                            cb();
                                        });
                                    });
                                }
                            ...
                            (err) => {
                                ...
                                if (err) { throw Error('goDownAndUpdateMainChainIndex eachSeries failed'); }
                                conn.query(
    ...
    function updateLatestIncludedMcIndex(last_main_chain_index, bRebuiltMc) {
        function checkAllLatestIncludedMcIndexesAreSet() {
            profiler.start();
            if (!_.isEqual(assocDbLimcisByUnit, assocLimcisByUnit)) { throwError(`different LIMCIs, mem: ${JSON.stringify(assocLimcisByUnit)}, db: ${JSON.stringify(assocDbLimcisByUnit)}`); }
            conn.query('SELECT unit FROM units WHERE latest_included_mc_index IS NULL AND level!=0', (rows) => {
                if (rows.length > 0) { throw Error(`${rows.length} units have latest_included_mc_index=NULL, e.g. unit ${rows[0].unit}`); }
            ...
            conn.query(
                ...
                */
                ...
                (rows) => {
                    ...
                    if (rows.length === 0) { return checkAllLatestIncludedMcIndexesAreSet(); }
                    profiler.start();
                    async.eachSeries(
                        rows,
                        (row, cb) => {
                            assocDbLimcisByUnit[row.unit] = row.latest_included_mc_index;
                            conn.query('UPDATE units SET latest_included_mc_index=? WHERE unit=?', [row.latest_included_mc_index, row.unit], () => { cb(); });
                        },
                        () => {
        ...
        function loadUnitProps(unit, handleProps) {
            if (storage.assocUnstableUnits[unit]) { return handleProps(storage.assocUnstableUnits[unit]); }
            storage.readUnitProps(conn, unit, handleProps);
        }

        function calcLIMCIs(onUpdated) {
            const arrFilledUnits = [];
            async.forEachOfSeries(
                ...
                (props, unit, cb) => {
                    ...
                    async.eachSeries(
                        props.parent_units,
                        (parent_unit, cb2) => {
                            loadUnitProps(parent_unit, (parent_props) => {
                                ...
                                if (parent_props.latest_included_mc_index === null) { return cb2('parent limci not known yet'); }
                                if (parent_props.latest_included_mc_index > max_limci) { max_limci = parent_props.latest_included_mc_index; }
                                cb2();
                            });
                        },
                        (err) => {
                            if (err) { return cb(); }
                            if (max_limci < 0) { throw Error(`max limci < 0 for unit ${unit}`); }
                ...
                () => {
                    arrFilledUnits.forEach((unit) => {
                        delete assocChangedUnits[unit];
                    });
                    if (Object.keys(assocChangedUnits).length > 0) { calcLIMCIs(onUpdated); } else { onUpdated(); }
                }
            );
        }
        ...
        calcLIMCIs(() => {
            conn.query('UPDATE units SET latest_included_mc_index=NULL WHERE main_chain_index>? OR main_chain_index IS NULL', [last_main_chain_index], (res) => {
                ...
                conn.query(
                    ...
                    */
                    ...
                    (rows) => {
                        ...
                        profiler.start();
                        if (rows.length === 0 && bRebuiltMc) { throw Error(`no latest_included_mc_index updated, last_mci=${last_main_chain_index}, affected=${res.affectedRows}`); }
                        async.eachSeries(
                            rows,
                            (row, cb) => {
                                console.log(row.main_chain_index, row.unit);
                                assocDbLimcisByUnit[row.unit] = row.main_chain_index;
                                conn.query('UPDATE units SET latest_included_mc_index=? WHERE unit=?', [row.main_chain_index, row.unit], () => { cb(); });
                            },
                            () => {
    ...
    function readLastStableMcUnit(handleLastStableMcUnit) {
        conn.query('SELECT unit FROM units WHERE is_on_main_chain=1 AND is_stable=1 ORDER BY main_chain_index DESC LIMIT 1', (rows) => {
            if (rows.length === 0) { throw Error('no units on stable MC?'); }
    ...
    function updateStableMcFlag() {
        ...
        profiler.start();
        readLastStableMcUnit((last_stable_mc_unit) => {
            console.log(`last stable mc unit ${last_stable_mc_unit}`);
            storage.readWitnesses(conn, last_stable_mc_unit, (arrWitnesses) => {
                conn.query('SELECT unit, is_on_main_chain, main_chain_index, level FROM units WHERE best_parent_unit=?', [last_stable_mc_unit], (rows) => {
                    if (rows.length === 0) {
                        // if (isGenesisUnit(last_stable_mc_unit))
                        //    return finish();
                        throw Error(`no best children of last stable MC unit ${last_stable_mc_unit}?`);
                    }
                    const arrMcRows = rows.filter(row => (row.is_on_main_chain === 1)); // only one element
                    const arrAltRows = rows.filter(row => (row.is_on_main_chain === 0));
                    if (arrMcRows.length !== 1) { throw Error('not a single MC child?'); }
                    const first_unstable_mc_unit = arrMcRows[0].unit;
                    const first_unstable_mc_index = arrMcRows[0].main_chain_index;
                    const first_unstable_mc_level = arrMcRows[0].level;
                    const arrAltBranchRootUnits = arrAltRows.map(row => row.unit);
                    ...
                    conn.query('SELECT witnessed_level FROM units WHERE is_free=1 AND is_on_main_chain=1', (wl_rows) => {
                        if (wl_rows.length !== 1) { throw Error('not a single mc wl'); }
                        // this is the level when we colect 7 witnesses if walking up the MC from its end
                        const mc_end_witnessed_level = wl_rows[0].witnessed_level;
                        conn.query(
                            ...
                            (min_wl_rows) => {
                                if (min_wl_rows.length !== 1) { throw Error('not a single min mc wl'); }
                                const min_mc_wl = min_wl_rows[0].min_mc_wl;
                                if (arrAltBranchRootUnits.length === 0) { // no alt branches
                                    if (min_mc_wl >= first_unstable_mc_level) { return advanceLastStableMcUnitAndTryNext(); }
                                    return finish();
                                    /*
                                ...
                                createListOfBestChildren(arrAltBranchRootUnits, (arrAltBestChildren) => {
                                    ...
                                    conn.query(
                                        ...
                                        (max_alt_rows) => {
                                            if (max_alt_rows.length !== 1) { throw Error('not a single max alt level'); }
                                            const max_alt_level = max_alt_rows[0].max_alt_level;
    ...
    function createListOfBestChildren(arrParentUnits, handleBestChildrenList) {
        if (arrParentUnits.length === 0) { return handleBestChildrenList([]); }
        const arrBestChildren = arrParentUnits.slice();

        function goDownAndCollectBestChildren(arrStartUnits, cb) {
            conn.query('SELECT unit, is_free FROM units WHERE best_parent_unit IN(?)', [arrStartUnits], (rows) => {
                ...
                async.eachSeries(
                    ...
                    (row, cb2) => {
                        ...
                        if (row.is_free === 1) { cb2(); } else { goDownAndCollectBestChildren([row.unit], cb2); }
                    },
                    cb
                );
            });
        ...
        goDownAndCollectBestChildren(arrParentUnits, () => {
...
function determineIfStableInLaterUnits(conn, earlier_unit, arrLaterUnits, handleResult) {
    if (storage.isGenesisUnit(earlier_unit)) { return handleResult(true); }
    // hack to workaround past validation error
    if (earlier_unit === 'LGFzduLJNQNzEqJqUXdkXr58wDYx77V8WurDF3+GIws=' && arrLaterUnits.join(',') === '6O4t3j8kW0/Lo7n2nuS8ITDv2UbOhlL9fF1M6j/PrJ4=') { return handleResult(true); }
    storage.readPropsOfUnits(conn, earlier_unit, arrLaterUnits, (objEarlierUnitProps, arrLaterUnitProps) => {
        if (objEarlierUnitProps.is_free === 1) { return handleResult(false); }
        const max_later_limci = Math.max.apply(
            null, arrLaterUnitProps.map(objLaterUnitProps => objLaterUnitProps.latest_included_mc_index));
        readBestParentAndItsWitnesses(conn, earlier_unit, (best_parent_unit, arrWitnesses) => {
            conn.query('SELECT unit, is_on_main_chain, main_chain_index, level FROM units WHERE best_parent_unit=?', [best_parent_unit], (rows) => {
                if (rows.length === 0) { throw Error(`no best children of ${best_parent_unit}?`); }
                const arrMcRows = rows.filter(row => (row.is_on_main_chain === 1)); // only one element
                const arrAltRows = rows.filter(row => (row.is_on_main_chain === 0));
                if (arrMcRows.length !== 1) { throw Error('not a single MC child?'); }
                const first_unstable_mc_unit = arrMcRows[0].unit;
                if (first_unstable_mc_unit !== earlier_unit) { throw Error('first unstable MC unit is not our input unit'); }
                const first_unstable_mc_index = arrMcRows[0].main_chain_index;
                const first_unstable_mc_level = arrMcRows[0].level;
                const arrAltBranchRootUnits = arrAltRows.map(row => row.unit);
                ...
                function findMinMcWitnessedLevel(handleMinMcWl) {
                    let min_mc_wl = Number.MAX_VALUE;
                    let count = 0;
                    ...
                    function goUp(start_unit) {
                        conn.query(
                            ...
                            (rows) => {
                                if (rows.length !== 1) { throw Error('findMinMcWitnessedLevel: not 1 row'); }
                                const row = rows[0];
                                if (row.count > 0 && row.witnessed_level < min_mc_wl) { min_mc_wl = row.witnessed_level; }
                                count += row.count;
                    ...
                    conn.query(
                        ...
                        (rows) => {
                            const row = rows[0];
                            if (row.count > 0) { min_mc_wl = row.witnessed_level; }
                            count += row.count;
                            goUp(row.best_parent_unit);
                        }
                    );
                ...
                function determineIfHasAltBranches(handleHasAltBranchesResult) {
                    if (arrAltBranchRootUnits.length === 0) { return handleHasAltBranchesResult(false); }
                    // check if alt branches are included by later units
                    async.eachSeries(
                        arrAltBranchRootUnits,
                        (alt_root_unit, cb) => {
                            graph.determineIfIncludedOrEqual(conn, alt_root_unit, arrLaterUnits, (bIncluded) => {
                ...
                function createListOfBestChildrenIncludedByLaterUnits(arrAltBranchRootUnits, handleBestChildrenList) {
                    if (arrAltBranchRootUnits.length === 0) { return handleBestChildrenList([]); }
                    const arrBestChildren = [];

                    function goDownAndCollectBestChildren(arrStartUnits, cb) {
                        conn.query('SELECT unit, is_free, main_chain_index FROM units WHERE best_parent_unit IN(?)', [arrStartUnits], (rows) => {
                            if (rows.length === 0) { return cb(); }
                            async.eachSeries(
                                rows,
                                (row, cb2) => {
                                    ...
                                    if (row.main_chain_index !== null && row.main_chain_index <= max_later_limci) { addUnit(); } else {
graph.determineIfIncludedOrEqual(conn, row.unit, arrLaterUnits, (bIncluded) => {
                    ...
                    function filterAltBranchRootUnits(cb) {
                        const arrFilteredAltBranchRootUnits = [];
                        conn.query('SELECT unit, is_free, main_chain_index FROM units WHERE unit IN(?)', [arrAltBranchRootUnits], (rows) => {
                            if (rows.length === 0) { throw Error('no alt branch root units?'); }
                            async.eachSeries(
                                rows,
                                (row, cb2) => {
                                    function addUnit() {
 ...
 graph.determineIfIncludedOrEqual(conn, row.unit, arrLaterUnits, (bIncluded) => {
                ...
                findMinMcWitnessedLevel((min_mc_wl) => {
                    // console.log("min mc wl", min_mc_wl);
                    determineIfHasAltBranches((bHasAltBranches) => {
                        if (!bHasAltBranches) {
                            // console.log("no alt");
                            if (min_mc_wl >= first_unstable_mc_level) { return handleResult(true); }
                        ...
                        // has alt branches
                        createListOfBestChildrenIncludedByLaterUnits(arrAltBranchRootUnits, (arrAltBestChildren) => {
...
function determineIfStableInLaterUnitsAndUpdateStableMcFlag(conn, earlier_unit, arrLaterUnits, bStableInDb, handleResult) {
    determineIfStableInLaterUnits(conn, earlier_unit, arrLaterUnits, (bStable) => {
        console.log('determineIfStableInLaterUnits', earlier_unit, arrLaterUnits, bStable);
        if (!bStable) { return handleResult(bStable); }
        if (bStable && bStableInDb) { return handleResult(bStable); }
        ...
        mutex.lock(['write'], (unlock) => {
            ...
            storage.readLastStableMcIndex(conn, (last_stable_mci) => {
                storage.readUnitProps(conn, earlier_unit, (objEarlierUnitProps) => {
                    const new_last_stable_mci = objEarlierUnitProps.main_chain_index;
                    ...
                    function advanceLastStableMcUnitAndStepForward() {
                        mci++;
                        if (mci <= new_last_stable_mci) { markMcIndexStable(conn, mci, advanceLastStableMcUnitAndStepForward); } else {
                            unlock();
...
function readBestParentAndItsWitnesses(conn, unit, handleBestParentAndItsWitnesses) {
    storage.readStaticUnitProps(conn, unit, (props) => {
        storage.readWitnesses(conn, props.best_parent_unit, (arrWitnesses) => {
...
function markMcIndexStable(conn, mci, onDone) {
    ...
    arrStabilizedUnits.forEach((unit) => {
        ...
        conn.query(
            ...
            (rows) => {
                async.eachSeries(
                    rows,
                    (row, cb) => {
                        if (row.sequence === 'final-bad') { return row.content_hash ? cb() : setContentHash(row.unit, cb); }
                        // temp-bad
                        if (row.content_hash) { throw Error('temp-bad and with content_hash?'); }
                        findStableConflictingUnits(row, (arrConflictingUnits) => {
                            const sequence = (arrConflictingUnits.length > 0) ? 'final-bad' : 'good';
                            console.log(`unit ${row.unit} has competitors ${arrConflictingUnits}, it becomes ${sequence}`);
                            conn.query('UPDATE units SET sequence=? WHERE unit=?', [sequence, row.unit], () => {
                                if (sequence === 'good') { conn.query('UPDATE inputs SET is_unique=1 WHERE unit=?', [row.unit], () => { cb(); }); } else { setContentHash(row.unit, cb); }
                            });
                        });
                    },
                    () => {
    ...
    function setContentHash(unit, onSet) {
        storage.readJoint(conn, unit, {
            ...
            ifFound(objJoint) {
                const content_hash = objectHash.getUnitContentHash(objJoint.unit);
                conn.query('UPDATE units SET content_hash=? WHERE unit=?', [content_hash, unit], () => {
                    onSet();
                });
            }
        });
    ...
    function findStableConflictingUnits(objUnitProps, handleConflictingUnits) {
        ...
        */
        conn.query(
            ...
            [objUnitProps.unit],
            (rows) => {
                const arrConflictingUnits = [];
                async.eachSeries(
                    rows,
                    (row, cb) => {
                        graph.compareUnitsByProps(conn, row, objUnitProps, (result) => {
                            if (result === null) { arrConflictingUnits.push(row.unit); }
                            cb();
                        });
                    },
                    () => {
  ...
  function addBalls() {
    conn.query(
      ...
      (unit_rows) => {
        if (unit_rows.length === 0) { throw Error(`no units on mci ${mci}`); }
        async.eachSeries(
          unit_rows,
          (objUnitProps, cb) => {
            ...
            conn.query(
              ...
              [unit],
              (parent_ball_rows) => {
                if (parent_ball_rows.some(parent_ball_row => (parent_ball_row.ball === null))) { throw Error(`some parent balls not found for unit ${unit}`); }
                const arrParentBalls = parent_ball_rows.map(parent_ball_row => parent_ball_row.ball);
                const arrSimilarMcis = getSimilarMcis(mci);
                const arrSkiplistUnits = [];
                const arrSkiplistBalls = [];
                if (objUnitProps.is_on_main_chain === 1 && arrSimilarMcis.length > 0) {
                  conn.query(
                    ...
                    (rows) => {
                      rows.forEach((row) => {
                        const skiplist_unit = row.unit;
                        const skiplist_ball = row.ball;
                        if (!skiplist_ball) { throw Error('no skiplist ball'); }
                ...
                function addBall() {
                  const ball = objectHash.getBallHash(unit, arrParentBalls, arrSkiplistBalls.sort(), objUnitProps.sequence === 'final-bad');
                  console.log(`ball=${ball}`);
                  if (objUnitProps.ball) { // already inserted
                    if (objUnitProps.ball !== ball) { throw Error(`stored and calculated ball hashes do not match, ball=${ball}, objUnitProps=${JSON.stringify(objUnitProps)}`); }
                    return cb();
                  }
                  conn.query('INSERT INTO balls (ball, unit) VALUES(?,?)', [ball, unit], () => {
                    conn.query('DELETE FROM hash_tree_balls WHERE ball=?', [ball], () => {
                      if (arrSkiplistUnits.length === 0) { return cb(); }
                      conn.query(
                        `INSERT INTO skiplist_units (unit, skiplist_unit) VALUES ${
                         arrSkiplistUnits.map(skiplist_unit => `(${conn.escape(unit)}, ${conn.escape(skiplist_unit)})`)}`,
                        () => { cb(); }
                      );
                    });
                  });
    ...
    function updateRetrievable() {
        storage.updateMinRetrievableMciAfterStabilizingMci(conn, mci, (min_retrievable_mci) => {
    ...
    function calcCommissions() {
        async.series([
            ...
            process.nextTick(() => { // don't call it synchronously with event emitter
...
function getSimilarMcis(mci) {
    const arrSimilarMcis = [];
    let divisor = 10;
    while (true) {
        if (mci % divisor === 0) {
            arrSimilarMcis.push(mci - divisor);
            divisor *= 10;
        } else { return arrSimilarMcis; }
    }
}

function throwError(msg) {
    if (typeof window === 'undefined') { throw Error(msg); } else { eventBus.emit('nonfatal_error', msg, new Error()); }
}
