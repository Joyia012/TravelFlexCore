        ...
        async.series([
            ...
            function (cb) {
                witnessProof.prepareWitnessProof(
                    ...
                    (err, arrUnstableMcJoints, arrWitnessChangeAndDefinitionJoints, _last_ball_unit, _last_ball_mci) => {
                        if (err) { return cb(err); }
                        objCatchupChain.unstable_mc_joints = arrUnstableMcJoints;
                        if (arrWitnessChangeAndDefinitionJoints.length > 0) { objCatchupChain.witness_change_and_definition_joints = arrWitnessChangeAndDefinitionJoints; }
            ...
            function (cb) { // jump by last_ball references until we land on or behind last_stable_mci
                ...
                function goUp(unit) {
                    storage.readJointWithBall(db, unit, (objJoint) => {
                        ...
                        storage.readUnitProps(db, unit, (objUnitProps) => {
...
function processCatchupChain(catchupChain, peer, callbacks) {
    ...
    witnessProof.processWitnessProof(
        ...
        (err, arrLastBallUnits, assocLastBallByLastBallUnit) => {
            ...
            if (arrLastBallUnits.indexOf(objFirstStableUnit.unit) === -1) { return callbacks.ifError('first stable unit is not last ball unit of any unstable unit'); }
            let last_ball_unit = objFirstStableUnit.unit;
            let last_ball = assocLastBallByLastBallUnit[last_ball_unit];
            if (objFirstStableJoint.ball !== last_ball) { return callbacks.ifError(`last ball and last ball unit do not match: ${objFirstStableJoint.ball}!==${last_ball}`); }
            ...
            for (let i = 0; i < catchupChain.stable_last_ball_joints.length; i++) {
                ...
                if (!objJoint.ball) { return callbacks.ifError('stable but no ball'); }
                if (!validation.hasValidHashes(objJoint)) { return callbacks.ifError('invalid hash'); }
                if (objUnit.unit !== last_ball_unit) { return callbacks.ifError('not the last ball unit'); }
                if (objJoint.ball !== last_ball) { return callbacks.ifError('not the last ball'); }
            ...
            async.series([
                function (cb) {
                    mutex.lock(['catchup_chain'], (_unlock) => {
                        unlock = _unlock;
                        db.query('SELECT 1 FROM catchup_chain_balls LIMIT 1', (rows) => {
                ...
                function (cb) { // adjust first chain ball if necessary and make sure it is the only stable unit in the entire chain
                    db.query(
                        ...
                        (rows) => {
                            if (rows.length === 0) {
                                if (storage.isGenesisBall(arrChainBalls[0])) { return cb(); }
                                return cb(`first chain ball ${arrChainBalls[0]} is not known`);
                            }
                            const objFirstChainBallProps = rows[0];
                            if (objFirstChainBallProps.is_stable !== 1) { return cb(`first chain ball ${arrChainBalls[0]} is not stable`); }
                            if (objFirstChainBallProps.is_on_main_chain !== 1) { return cb(`first chain ball ${arrChainBalls[0]} is not on mc`); }
                            storage.readLastStableMcUnitProps(db, (objLastStableMcUnitProps) => {
                                const last_stable_mci = objLastStableMcUnitProps.main_chain_index;
                                ...
                                if (!arrChainBalls[1]) { return cb(); }
                                db.query('SELECT is_stable FROM balls JOIN units USING(unit) WHERE ball=?', [arrChainBalls[1]], (rows2) => {
                                    if (rows2.length === 0) { return cb(); }
                                    const objSecondChainBallProps = rows2[0];
                                    if (objSecondChainBallProps.is_stable === 1) { return cb(`second chain ball ${arrChainBalls[1]} must not be stable`); }
                                    cb();
                                });
                            });
                        }
                    ...
                    async.eachSeries(
                        ...
                        (objBall, cb) => {
                            ...
                            conn.query('SELECT ball FROM hash_tree_balls WHERE ball IN(?)', [objBall.parent_balls], (rows) => {
                                ...
                                const arrFoundBalls = rows.map(row => row.ball);
...
function purgeHandledBallsFromHashTree(conn, onDone) {
    conn.query('SELECT ball FROM hash_tree_balls CROSS JOIN balls USING(ball)', (rows) => {
        ...
        const arrHandledBalls = rows.map(row => row.ball);
