...
function pickParentUnits(conn, arrWitnesses, onDone) {
  ...
  conn.query(
    ...
    [arrWitnesses],
    (rows) => {
      if (rows.some(row => (row.version !== constants.version || row.alt !== constants.alt))) { throw Error('wrong network'); }
      ...
      if (rows.filter(row => (row.count_matching_witnesses >= count_required_matches)).length === 0) { return pickDeepParentUnits(conn, arrWitnesses, null, onDone); }
...
function adjustParentsToNotRetreatWitnessedLevel(conn, arrWitnesses, arrParentUnits, handleAdjustedParents) {
    ...
    function replaceExcludedParent(arrCurrentParentUnits, excluded_unit) {
        ...
        conn.query('SELECT DISTINCT parent_unit FROM parenthoods WHERE child_unit IN(?)', [arrNewExcludedUnits], (rows) => {
            const arrCandidateReplacements = rows.map(row => row.parent_unit);
            ...
            conn.query(
                ...
                (rows) => {
                    const arrCandidatesWithOtherChildren = rows.map(row => row.parent_unit);
                    ...
                    if (arrNewParents.length === 0) { throw Error('no new parents'); }
    ...
    function checkWitnessedLevelAndReplace(arrCurrentParentUnits) {
        ...
        if (iterations > 0 && arrExcludedUnits.length === 0) { throw Error('infinite cycle'); }
        iterations++;
        determineWitnessedLevels(conn, arrWitnesses, arrCurrentParentUnits, (child_witnessed_level, best_parent_witnessed_level, best_parent_unit) => {
            if (child_witnessed_level >= best_parent_witnessed_level) { return handleAdjustedParents(arrCurrentParentUnits.sort()); }
...
function pickParentUnitsUnderWitnessedLevel(conn, arrWitnesses, max_wl, onDone) {
    ...
    conn.query(
        ...
        (rows) => {
            if (rows.length === 0) { return pickDeepParentUnits(conn, arrWitnesses, max_wl, onDone); }
            const arrParentUnits = rows.map(row => row.unit).sort();
...
function pickDeepParentUnits(conn, arrWitnesses, max_wl, onDone) {
    ...
    conn.query(
        ...
        (rows) => {
            if (rows.length === 0) { return onDone('failed to find compatible parents: no deep units'); }
            const arrParentUnits = rows.map(row => row.unit);
            checkWitnessedLevelNotRetreatingAndLookLower(conn, arrWitnesses, arrParentUnits, true, onDone);
        }
    );
...
function checkWitnessedLevelNotRetreatingAndLookLower(conn, arrWitnesses, arrParentUnits, bRetryDeeper, onDone) {
    determineWitnessedLevels(conn, arrWitnesses, arrParentUnits, (child_witnessed_level, best_parent_witnessed_level) => {
        if (child_witnessed_level >= best_parent_witnessed_level) { return onDone(null, arrParentUnits); }
...
function findLastStableMcBall(conn, arrWitnesses, onDone) {
    conn.query(
        ...
        [arrWitnesses, constants.COUNT_WITNESSES - constants.MAX_WITNESS_LIST_MUTATIONS],
        (rows) => {
            if (rows.length === 0) { return onDone('failed to find last stable ball'); }
  ...
  main_chain.determineIfStableInLaterUnits(conn, last_stable_mc_ball_unit, arrParentUnits, (bStable) => {
    if (bStable) {
      conn.query('SELECT ball, main_chain_index FROM units JOIN balls USING(unit) WHERE unit=?', [last_stable_mc_ball_unit], (rows) => {
        if (rows.length !== 1) { throw Error(`not 1 ball by unit ${last_stable_mc_ball_unit}`); }
        const row = rows[0];
    ...
    if (arrParentUnits.length > 1) { // select only one parent
      pickDeepParentUnits(conn, arrWitnesses, null, (err, arrAdjustedParentUnits) => {
        if (err) { throw Error(`pickDeepParentUnits in adjust failed: ${err}`); }
    ...
    storage.readStaticUnitProps(conn, last_stable_mc_ball_unit, (objUnitProps) => {
      if (!objUnitProps.best_parent_unit) { throw Error(`no best parent of ${last_stable_mc_ball_unit}`); }
...
function trimParentList(conn, arrParentUnits, arrWitnesses, handleTrimmedList) {
    if (arrParentUnits.length <= constants.MAX_PARENTS_PER_UNIT) { return handleTrimmedList(arrParentUnits); }
    conn.query(
        ...
        (rows) => {
            handleTrimmedList(rows.map(row => row.unit).sort());
        }
    );
}
...
function pickParentUnitsAndLastBall(conn, arrWitnesses, onDone) {
    pickParentUnits(conn, arrWitnesses, (err, arrParentUnits) => {
        ...
        findLastStableMcBall(conn, arrWitnesses, (err, last_stable_mc_ball, last_stable_mc_ball_unit, last_stable_mc_ball_mci) => {
            if (err) { return onDone(err); }
            adjustLastStableMcBallAndParents(
                conn, last_stable_mc_ball_unit, arrParentUnits, arrWitnesses,
                (last_stable_ball, last_stable_unit, last_stable_mci, arrAdjustedParentUnits) => {
                    trimParentList(conn, arrAdjustedParentUnits, arrWitnesses, (arrTrimmedParentUnits) => {
                        storage.findWitnessListUnit(conn, arrWitnesses, last_stable_mci, (witness_list_unit) => {
                            ...
                            if (witness_list_unit) { objFakeUnit.witness_list_unit = witness_list_unit; }
                            storage.determineIfHasWitnessListMutationsAlongMc(conn, objFakeUnit, last_stable_unit, arrWitnesses, (err) => {
                                if (err) { return onDone(err); } // if first arg is not array, it is error
