...
function validatePrivatePayment(conn, objPrivateElement, objPrevPrivateElement, callbacks) {
    function validateSpendProof(spend_proof, cb) {
        ...
        conn.query(
            ...
            (rows) => {
                ...
                if (rows.length !== 1) { return cb(`expected 1 spend proof, found ${rows.length}`); }
                const stored_spend_proof = rows[0].spend_proof;
                const spend_proof_address = rows[0].address;
                if (stored_spend_proof !== spend_proof) { return cb("spend proof doesn't match"); }
                if (objPrevPrivateElement && objPrevPrivateElement.output.address !== spend_proof_address) { return cb('spend proof address does not match src output'); }
                if (input.address && input.address !== spend_proof_address) { return cb('spend proof address does not match issuer address'); }
                cb();
            }
        );
    }
    ...
    function validateSourceOutput(cb) {
        if (conf.bLight) { return cb(); } // already validated the linkproof
        profiler.start();
    ...
    if (!ValidationUtils.isStringOfLength(payload.asset, constants.HASH_LENGTH)) { return callbacks.ifError('invalid asset in private payment'); }
    if (!ValidationUtils.isPositiveInteger(payload.denomination)) { return callbacks.ifError('invalid denomination in private payment'); }
    if (!ValidationUtils.isNonemptyObject(objPrivateElement.output)) { return callbacks.ifError('no output'); }
    if (!ValidationUtils.isNonnegativeInteger(objPrivateElement.output_index)) { return callbacks.ifError('invalid output index'); }
    if (!ValidationUtils.isNonemptyArray(payload.outputs)) { return callbacks.ifError('invalid outputs'); }
    const our_hidden_output = payload.outputs[objPrivateElement.output_index];
    if (!ValidationUtils.isNonemptyObject(payload.outputs[objPrivateElement.output_index])) { return callbacks.ifError('no output at output_index'); }
    if (!ValidationUtils.isValidAddress(objPrivateElement.output.address)) { return callbacks.ifError('bad address in output'); }
    if (!ValidationUtils.isNonemptyString(objPrivateElement.output.blinding)) { return callbacks.ifError('bad blinding in output'); }
    if (objectHash.getBase64Hash(objPrivateElement.output) !== our_hidden_output.output_hash) { return callbacks.ifError(`output hash doesn't match, output=${JSON.stringify(objPrivateElement.output)}, hash=${our_hidden_output.output_hash}`); }
    if (!ValidationUtils.isArrayOfLength(payload.inputs, 1)) { return callbacks.ifError('inputs array must be 1 element long'); }
    var input = payload.inputs[0];
    if (!ValidationUtils.isNonemptyObject(input)) { return callbacks.ifError('no inputs[0]'); }
    ...
    validation.initPrivatePaymentValidationState(
        ...
        (bStable, objPartialUnit, objValidationState) => {
            ...
            if (!input.type) { // transfer
                if (typeof input.unit !== 'string') { return callbacks.ifError('invalid unit in private payment'); }
                if (!ValidationUtils.isNonnegativeInteger(input.message_index)) { return callbacks.ifError('invalid input message_index'); }
                if (!ValidationUtils.isNonnegativeInteger(input.output_index)) { return callbacks.ifError('invalid input output_index'); }
                if (!objPrevPrivateElement || !objPrevPrivateElement.output || !objPrevPrivateElement.output.blinding) { return callbacks.ifError('no prev output blinding'); }
                if (!objPrevPrivateElement.payload || !objPrevPrivateElement.payload.outputs) { return callbacks.ifError('no prev outputs'); }
                ...
                if (!prev_hidden_output) { return callbacks.ifError('no prev hidden output'); }
                ...
                if (objPrevPrivateElement) { return callbacks.ifError('prev payload and initial input'); }
            ...
            } else { return callbacks.ifError('neither transfer nor issue in private input'); }

            if (!objPartialUnit.authors.some(author => (author.address === input_address))) { return callbacks.ifError('input address not found among unit authors'); }
...
function parsePrivatePaymentChain(conn, arrPrivateElements, callbacks) {
    ...
    if (!issuePrivateElement.payload || !issuePrivateElement.payload.inputs || !issuePrivateElement.payload.inputs[0]) { return callbacks.ifError('invalid issue private element'); }
    const asset = issuePrivateElement.payload.asset;
    if (!asset) { return callbacks.ifError('no asset in issue private element'); }
    const denomination = issuePrivateElement.payload.denomination;
    if (!denomination) { return callbacks.ifError('no denomination in issue private element'); }
    async.forEachOfSeries(
        arrPrivateElements,
        (objPrivateElement, i, cb) => {
            if (!objPrivateElement.payload || !objPrivateElement.payload.inputs || !objPrivateElement.payload.inputs[0]) { return cb('invalid payload'); }
            if (!objPrivateElement.output) { return cb('no output in private element'); }
            if (objPrivateElement.payload.asset !== asset) { return cb('private element has a different asset'); }
            if (objPrivateElement.payload.denomination !== denomination) { return cb('private element has a different denomination'); }
            var prevElement = null;
            if (i + 1 < arrPrivateElements.length) { // excluding issue transaction
                var prevElement = arrPrivateElements[i + 1];
                if (prevElement.unit !== objPrivateElement.payload.inputs[0].unit) { return cb('not referencing previous element unit'); }
                if (prevElement.message_index !== objPrivateElement.payload.inputs[0].message_index) { return cb('not referencing previous element message index'); }
                if (prevElement.output_index !== objPrivateElement.payload.inputs[0].output_index) { return cb('not referencing previous element output index'); }
            }
...
conn.addQuery(arrQueries,
 ...
 conn.addQuery(arrQueries,
                        ...
                        payload.asset, payload.denomination, input_address, is_unique]);
} else { throw Error('neither transfer nor issue after validation'); }
                ...
                (row, cb) => {
                    ...
                    function goUp(unit, message_index) {
                        ...
                        conn.query(
                            ...
                            [unit, message_index],
                            (src_rows) => {
                                if (src_rows.length === 0) { throw Error('updating unstable: blackbyte input not found'); }
                                if (src_rows.length > 1) { throw Error('updating unstable: more than one input found'); }
                                ...
                                conn.query(
                                    ...
                                    (prev_rows) => {
                                        if (prev_rows.length === 0) { throw Error('src unit not found'); }
                                        const prev_output = prev_rows[0];
                                        if (prev_output.is_serial === 0) { throw Error('prev is already nonserial'); }
                                        if (prev_output.is_stable === 0) { throw Error('prev is not stable'); }
                                        if (prev_output.is_serial === 1 && prev_output.sequence !== 'good') { throw Error('prev is_serial=1 but seq!=good'); }
                    ...
                    updateOutputProps(row.unit, is_serial, () => {
...
function pickIndivisibleCoinsForAmount(
    ...
    if (!ValidationUtils.isPositiveInteger(amount)) { throw Error(`bad amount: ${amount}`); }
    updateIndivisibleOutputsThatWereReceivedUnstable(conn, () => {
        ...
        function createOutputs(amount_to_use, change_amount) {
            ...
            if (objAsset.is_private) { output.blinding = composer.generateBlinding(); }
            ...
            if (change_amount) {
                ...
                if (objAsset.is_private) { change_output.blinding = composer.generateBlinding(); }
                outputs.push(change_output);
                outputs.sort((o1, o2) => (o1.address < o2.address) ? -1 : 1);
            }
            return outputs;
        }
        ...
        function pickNextCoin(remaining_amount) {
            ...
            if (remaining_amount <= 0) { throw Error(`remaining amount is ${remaining_amount}`); }
            conn.query(
                ...
                (rows) => {
                    if (rows.length === 0) { return issueNextCoinIfAllowed(remaining_amount); }
                    const row = rows[0];
                    ...
                    if (objAsset.is_private) {
                        ...
                        if (bMultiAuthored) { objSpendProof.address = row.address; }
                    ...
                    if (accumulated_amount >= amount - tolerance_minus && accumulated_amount <= amount + tolerance_plus) { return onDone(null, arrPayloadsWithProofs); }
        ...
        function issueNextCoin(remaining_amount) {
            ...
            conn.query(
                ...
                (rows) => {
                    if (rows.length === 0) { return onDone(NOT_ENOUGH_FUNDS_ERROR_MESSAGE); }
                    const row = rows[0];
                    if (!!row.count_coins !== !!objAsset.cap) { throw Error('invalid asset cap and count_coins'); }
                    ...
                    conn.query(
                        ...
                        () => {
                            ...
                            if (bMultiAuthored) { input.address = issuer_address; }
                            let amount_to_use;
                            let change_amount;
                            ...
                            } else { amount_to_use = issue_amount; }
                            const payload = {
                                asset,
                                denomination,
                                inputs: [input],
                            ...
                            if (objAsset.is_private) {
                                ...
                                if (bMultiAuthored) { objSpendProof.address = issuer_address; }
                                objPayloadWithProof.spend_proof = objSpendProof;
                            }
                            arrPayloadsWithProofs.push(objPayloadWithProof);
                            accumulated_amount += amount_to_use;
        ...
        if (objAsset && objAsset.auto_destroy) {
            ...
            if (i >= 0) { arrSpendableAddresses.splice(i, 1); }
        }
        if (arrSpendableAddresses.length > 0) { pickNextCoin(amount); } else { issueNextCoinIfAllowed(amount); }
    });
}
...
function buildPrivateElementsChain(conn, unit, message_index, output_index, payload, handlePrivateElements) {
    ...
    hidden_payload.outputs.forEach((o) => {
        delete o.address;
        delete o.blinding;
        ...
        conn.query(
            ...
            (in_rows) => {
                ...
                if (in_row.src_unit) { // transfer
                    ...
                    if (in_row.count_authors > 1) { input.address = in_row.address; }
                }
                conn.query(
                    ...
                    (out_rows) => {
                        if (out_rows.length === 0) { throw Error('blackbyte output not found'); }
                        const output = {};
                        const outputs = out_rows.map((o) => {
                            if (o.asset !== asset) { throw Error('outputs asset mismatch'); }
                            if (o.denomination !== denomination) { throw Error('outputs denomination mismatch'); }
                        ...
                        if (!output.address) { throw Error('output not filled'); }
...
function composeIndivisibleAssetPaymentJoint(params) {
  ...
  if ((params.to_address || params.amount) && params.asset_outputs) { throw Error('to_address and asset_outputs at the same time'); }
  if (params.asset_outputs && params.asset_outputs.length !== 1) { throw Error('multiple indivisible asset outputs not supported'); }
  if (!ValidationUtils.isNonemptyArray(params.fee_paying_addresses)) { throw Error('no fee_paying_addresses'); }
  let arrBaseOutputs = [{ address: params.fee_paying_addresses[0], amount: 0 }]; // public outputs: the change only
  if (params.base_outputs) { arrBaseOutputs = arrBaseOutputs.concat(params.base_outputs); }
  composer.composeJoint({
    ...
    retrieveMessages: function createAdditionalMessages(conn, last_ball_mci, bMultiAuthored, arrPayingAddresses, onDone) {
      ...
      storage.loadAssetWithListOfAttestedAuthors(conn, params.asset, last_ball_mci, arrAssetPayingAddresses, (err, objAsset) => {
        ...
        if (!objAsset.fixed_denominations) { return onDone('divisible asset type'); }
        if (!objAsset.is_transferrable && params.to_address !== objAsset.definer_address && arrAssetPayingAddresses.indexOf(objAsset.definer_address) === -1) { return onDone('the asset is not transferrable and definer not found on either side of the deal'); }
        if (objAsset.cosigned_by_definer && arrPayingAddresses.concat(params.signing_addresses || []).indexOf(objAsset.definer_address) === -1) { return onDone('the asset must be cosigned by definer'); }
        if (objAsset.spender_attested && objAsset.arrAttestedAddresses.length === 0) { return onDone('none of the authors is attested'); }

        const target_amount = params.to_address ? params.amount : params.asset_outputs[0].amount;
        const to_address = params.to_address ? params.to_address : params.asset_outputs[0].address;
 ...
 return onDone({
            ...
            for (let i = 0; i < arrPayloadsWithProofs.length; i++) {
              ...
              } else { payload_hash = objectHash.getBase64Hash(payload); }
              const objMessage = {
                app: 'payment',
              ...
              } else { objMessage.payload = payload; }
        ...
        ifOk(objJoint, assocPrivatePayloads, composer_unlock) {
            ...
            validation.validate(objJoint, {
                ...
                ifOk(objValidationState, validation_unlock) {
                        ...
                        preCommitCallback = function (conn, cb) {
                            async.eachSeries(
                                            ...
                                            buildPrivateElementsChain(
                                                ...
                                                (arrPrivateElements) => {
                                                    validateAndSavePrivatePaymentChain(conn, _.cloneDeep(arrPrivateElements), {
                                                        ...
                                                        ifOk() {
                                                            if (output.address === to_address) { arrRecipientChains.push(arrPrivateElements); }
                                ...
                                (err) => {
                                    ...
                                    const onSuccessfulPrecommit = !conf.bLight ? cb : function () {
                                        composer.postJointToLightVendorIfNecessaryAndSave(
                                            ...
                                            () => { // not actually saving yet but greenlighting the commit
                                                cb();
                                            }
                                        );
                                    };
                                    if (!callbacks.preCommitCb) { return onSuccessfulPrecommit(); }
                                    callbacks.preCommitCb(conn, arrRecipientChains, arrCosignerChains, onSuccessfulPrecommit);
                                }
                            );
                        };
                    } else if (typeof callbacks.preCommitCb === 'function') {
                            preCommitCallback = function (conn, cb) {
                                callbacks.preCommitCb(conn, objJoint, cb);
                            };
                        }

                    const saveAndUnlock = function () {
                        writer.saveJoint(
                            ...
                            (err) => {
                                ...
                                validation_unlock();
                                composer_unlock();
                                if (bPreCommitCallbackFailed) { callbacks.ifError(`precommit callback failed: ${err}`); } else { callbacks.ifOk(objJoint, arrRecipientChains, arrCosignerChains); }
                            }
                        );
                    };
                    ...
                    if (bPrivate || !conf.bLight) { return saveAndUnlock(); }
                    composer.postJointToLightVendorIfNecessaryAndSave(
                        objJoint,
                        (err) => { // light only
                            console.log(`failed to post indivisible payment ${unit}`);
            ...
            async.eachSeries(
                ...
                (row, cb) => {
                    ...
                    db.query(
                        ...
                        [unit, message_index],
                        (input_rows) => {
                            if (input_rows.length !== 1) { throw Error('not 1 input'); }
                            const input_row = input_rows[0];
                            if (input_row.asset !== asset) { throw Error("assets don't match"); }
                            if (input_row.denomination !== row.denomination) { throw Error("denominations don't match"); }
                            if (input_row.src_message_index === null || input_row.src_output_index === null) { throw Error('only transfers supported'); }
                            const input = {
                            ...
                            db.query(
                                ...
                                (outputs) => {
                                    if (outputs.length === 0) { throw Error(`outputs not found for mi ${message_index}`); }
                                    if (!outputs.some(output => (output.address && output.blinding))) { throw Error('all outputs are hidden'); }
                                    ...
                                    if (payload_hash !== row.payload_hash) { throw Error('wrong payload hash'); }
                                    async.forEachOfSeries(
                                        payload.outputs,
                                        (output, output_index, cb3) => {
...
// to_address: to_address, change_address: change_address, amount: amount, tolerance_plus: tolerance_plus, tolerance_minus: tolerance_minus, signer: signer, callbacks: callbacks}
    ...
    db.query(
        ...
        (rows) => {
            for (let i = 0; i < rows.length; i++) {
                const row = rows[i];
                if (row.denomination > remaining_amount) { continue; }
...
// {asset: asset, available_paying_addresses: arrAvailablePayingAddresses,
// available_fee_paying_addresses: arrAvailableFeePayingAddresses, to_address: to_address,
// change_address: change_address, amount: amount, tolerance_plus: tolerance_plus, tolerance_minus: tolerance_minus, signer: signer, callbacks: callbacks}
function composeMinimalIndivisibleAssetPaymentJoint(params) {
    if (!ValidationUtils.isNonemptyArray(params.available_paying_addresses)) { throw Error('no available_paying_addresses'); }
    if (!ValidationUtils.isNonemptyArray(params.available_fee_paying_addresses)) { throw Error('no available_fee_paying_addresses'); }
    readFundedAddresses(
        ...
        (arrFundedPayingAddresses, arrFundedFeePayingAddresses) => {
            if (arrFundedPayingAddresses.length === 0) { return params.callbacks.ifNotEnoughFunds('all paying addresses are unfunded in asset, make sure all your funds are confirmed'); }
...
// {asset: asset, available_paying_addresses: arrAvailablePayingAddresses,
// available_fee_paying_addresses: arrAvailableFeePayingAddresses,
// to_address: to_address, amount: amount, tolerance_plus: tolerance_plus,
// tolerance_minus: tolerance_minus, signer: signer, callbacks: callbacks}
...
function getToAddress(params) {
    if (params.to_address) { return params.to_address; }
    if (params.asset_outputs) { return params.asset_outputs[0].address; }
