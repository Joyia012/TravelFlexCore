    ...
    async.series([
        ...
        function (cb) { // collect all unstable MC units
            ...
            db.query(
                ...
                (rows) => {
                    async.eachSeries(rows, (row, cb2) => {
                        storage.readJointWithBall(db, row.unit, (objJoint) => {
                            ...
                            for (let i = 0; i < objJoint.unit.authors.length; i++) {
                                ...
                                if (arrWitnesses.indexOf(address) >= 0 && arrFoundWitnesses.indexOf(address) === -1) { arrFoundWitnesses.push(address); }
                            }
                            // collect last balls of majority witnessed units
                            // (genesis lacks last_ball_unit)
                            if (objJoint.unit.last_ball_unit && arrFoundWitnesses.length >= constants.MAJORITY_OF_WITNESSES) { arrLastBallUnits.push(objJoint.unit.last_ball_unit); }
                            cb2();
                        });
                    }, cb);
                }
        ...
        function (cb) { // select the newest last ball unit
            if (arrLastBallUnits.length === 0) { return cb('your witness list might be too much off, too few witness authored units'); }
...
function processWitnessProof(arrUnstableMcJoints, arrWitnessChangeAndDefinitionJoints, bFromCurrent, handleResult) {
    myWitnesses.readMyWitnesses((arrWitnesses) => {
        ...
        for (var i = 0; i < arrUnstableMcJoints.length; i++) {
            ...
            if (objJoint.ball) { return handleResult('unstable mc but has ball'); }
            if (!validation.hasValidHashes(objJoint)) { return handleResult('invalid hash'); }
            if (arrParentUnits && arrParentUnits.indexOf(objUnit.unit) === -1) { return handleResult('not in parents'); }
            ...
            for (var j = 0; j < objUnit.authors.length; j++) {
                ...
                if (arrWitnesses.indexOf(address) >= 0) {
                    if (arrFoundWitnesses.indexOf(address) === -1) { arrFoundWitnesses.push(address); }
                    if (!bAddedJoint) { arrWitnessJoints.push(objJoint); }
                    bAddedJoint = true;
                }
            }
        ...
        if (arrFoundWitnesses.length < constants.MAJORITY_OF_WITNESSES) { return handleResult('not enough witnesses'); }


        if (arrLastBallUnits.length === 0) { throw Error('processWitnessProof: no last ball units'); }
        ...
        for (var i = 0; i < arrWitnessChangeAndDefinitionJoints.length; i++) {
            ...
            var objUnit = objJoint.unit;
            if (!objJoint.ball) { return handleResult('witness_change_and_definition_joints: joint without ball'); }
            if (!validation.hasValidHashes(objJoint)) { return handleResult('witness_change_and_definition_joints: invalid hash'); }
            let bAuthoredByWitness = false;
            for (var j = 0; j < objUnit.authors.length; j++) {
                var address = objUnit.authors[j].address;
                if (arrWitnesses.indexOf(address) >= 0) { bAuthoredByWitness = true; }
            }
            if (!bAuthoredByWitness) { return handleResult('not authored by my witness'); }
        }
        ...
        function validateUnit(objUnit, bRequireDefinitionOrChange, cb2) {
            ...
            async.eachSeries(
                ...
                (author, cb3) => {
                    ...
                    if (!definition_chash) { throw Error(`definition chash not known for address ${address}`); }
                    if (author.definition) {
                        if (objectHash.getChash160(author.definition) !== definition_chash) { return cb3("definition doesn't hash to the expected value"); }
                    ...
                    if (assocDefinitions[definition_chash]) { return handleAuthor(); }
                ...
                (err) => {
                    ...
                    if (bRequireDefinitionOrChange && !bFound) { return cb2('neither definition nor change'); }
                    cb2();
                }
            ); // each authors
        }
            ...
            function (cb) { // handle changes of definitions
                async.eachSeries(
                    ...
                    (objJoint, cb2) => {
                        ...
                        if (!bFromCurrent) { return validateUnit(objUnit, true, cb2); }
                        db.query('SELECT 1 FROM units WHERE unit=? AND is_stable=1', [objUnit.unit], (rows) => {
