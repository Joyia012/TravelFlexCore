...
function getMerkleProof(arrElements, element_index) {
    ...
    if (index < 0 || index >= arrElements.length) { throw Error('invalid index'); }
    let arrHashes = arrElements.map(hash);
    const arrSiblings = [];
    while (arrHashes.length > 1) {
        const arrOverHashes = []; // hashes over hashes
        ...
        arrHashes = arrOverHashes;
        if (overIndex === null) { throw Error('overIndex not defined'); }
        index = overIndex;
    }
    // add merkle root
...
function serializeMerkleProof(proof) {
    let serialized_proof = proof.index;
    if (proof.siblings.length > 0) { serialized_proof += `-${proof.siblings.join('-')}`; }
    serialized_proof += `-${proof.root}`;
    return serialized_proof;
}
...
function verifyMerkleProof(element, proof) {
    ...
    for (let i = 0; i < proof.siblings.length; i++) {
        ...
        if (index % 2 === 0) { the_other_sibling = hash(the_other_sibling + proof.siblings[i]); } else { the_other_sibling = hash(proof.siblings[i] + the_other_sibling); }
