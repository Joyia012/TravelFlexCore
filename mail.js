...
if (conf.smtpTransport === 'relay' && !conf.smtpRelay) { throw Error('please set smtpRelay in conf'); }
...
function sendMailThroughUnixSendmail(params, cb) {
    const child = child_process.spawn('/usr/sbin/sendmail', ['-t', params.to]);
...
function sendMailDirectly(params, cb) {
    ...
    DNS.resolveMx(hostname, (err, exchanges) => {
        ...
        if (exchanges && exchanges.length) { exchange = exchanges[0].exchange; }
