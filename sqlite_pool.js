...
module.exports = function (db_name, MAX_CONNECTIONS, bReadOnly) {
    ...
    function addQuery(arr) {
        ...
        arr.push((callback) => { // add callback for async.series() member tasks
            if (typeof query_args[query_args.length - 1] !== 'function') { query_args.push(() => { callback(); }); } // add callback
            else {
...
if (!arrConnections[i].bInUse) {
 ...
 if (arrConnections[i].bInUse) { count++; }
    ...
    function escape(str) {
        if (typeof str === 'string') { return `'${str.replace(/'/g, "''")}'`; } else if (Array.isArray(str)) { return str.map(member => escape(member)).join(','); }
